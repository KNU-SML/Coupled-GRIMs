...
CONTAINS
   ...
   SUBROUTINE dyn_ldf_lap_lf_t( kt, Kbb, Kmm, pu, pv, ktuv, pu_rhs, pv_rhs, ktuv_rhs, kpass )
      ...
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktuv+1:,(ntsj-nn_hls-1)*ktuv+1:    ,:), INTENT(in   ) ::   pu, pv ! before velocity  [m/s]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktuv_rhs+1:,(ntsj-nn_hls-1)*ktuv_rhs+1:,:), INTENT(inout) ::   pu_rhs, pv_rhs   ! velocity trend   [m/s2]
      !
      ...
      CASE ( np_typ_rot )       !==  Vorticity-Divergence operator  ==!
         !
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)                           ! Horizontal slab
         ...
         END DO   ;   END DO   ;   END DO                                                  !   End of slab
         !
   ...
   SUBROUTINE dyn_ldf_blp_lf( kt, Kbb, Kmm, pu, pv, pu_rhs, pv_rhs )
      ...
      !
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) ::   zulap, zvlap   ! laplacian at u- and v-point
      !!----------------------------------------------------------------------
      !
      IF( kt == nit000 )  THEN
         IF(lwp) WRITE(numout,*)
