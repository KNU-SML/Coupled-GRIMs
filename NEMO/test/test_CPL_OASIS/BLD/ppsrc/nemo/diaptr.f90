...
CONTAINS
   ...
   SUBROUTINE dia_ptr( kt, Kmm, pvtr )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk)    , INTENT(in), OPTIONAL ::   pvtr   ! j-effective transport
      !!----------------------------------------------------------------------
      !
   ...
   SUBROUTINE dia_ptr_zint( Kmm, pvtr )
      ...
      IF( PRESENT( pvtr ) ) THEN
         ...
         IF( iom_use( 'zomsf' ) .OR. iom_use( 'sopstove' ) .OR. iom_use( 'sophtove' ) ) THEN
            ALLOCATE( v_msf(ntsj-(nn_hls):ntej+(nn_hls),jpk,nbasin) )
         ...
         IF(  iom_use( 'sopstove' ) .OR. iom_use( 'sophtove' ) .OR.   &
            ...
            ALLOCATE( zmask(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk), zts(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk,jpts), &
               &      sjk(ntsj-(nn_hls):ntej+(nn_hls),jpk,nbasin), &
               &      zt_jk(ntsj-(nn_hls):ntej+(nn_hls),jpk,nbasin), zs_jk(ntsj-(nn_hls):ntej+(nn_hls),jpk,nbasin) )

            zmask(:,:,:) = 0._wp
            zts(:,:,:,:) = 0._wp

            DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  0) ; DO ji = ntsi-( 1), ntei+(  1)
      ...
      ELSE
         ...
         IF( iom_use( 'zotem' ) .OR. iom_use( 'zosal' ) .OR. iom_use( 'zosrf' )  ) THEN
            ...
            DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
         ...
         IF( iom_use( 'sopstvtr' ) .OR. iom_use( 'sophtvtr' ) ) THEN
            ALLOCATE( zts(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk,jpts) )
   ...
   SUBROUTINE dia_ptr_hst( ktra, cptr, pvflx )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk)    , INTENT(in)   :: pvflx ! 3D input array of advection/diffusion
      REAL(wp), DIMENSION(ntsj-(nn_hls):ntej+(nn_hls),nbasin)                 :: zsj   !
      INTEGER                                        :: jn    !
   ...
   SUBROUTINE ptr_sum_2d( phstr, pva )
      ...
      REAL(wp), DIMENSION(ntsj-(nn_hls):ntej+(nn_hls),nbasin), INTENT(in)            ::  pva    !
      INTEGER                                               ::  jj
   ...
   SUBROUTINE ptr_sum_3d( phstr, pva )
      ...
      REAL(wp), DIMENSION(ntsj-(nn_hls):ntej+(nn_hls),jpk,nbasin), INTENT(in)        ::  pva    !
      INTEGER                                               ::  jj, jk
      INTEGER, DIMENSION(1)              ::  ish1d
   ...
   FUNCTION ptr_sj_3d( pvflx, pmsk )   RESULT ( p_fval )
      ...
      !!----------------------------------------------------------------------
      REAL(wp), INTENT(in), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk)  ::   pvflx  ! mask flux array at V-point
      REAL(wp), INTENT(in), DIMENSION(jpi,jpj)  ::   pmsk   ! Optional 2D basin mask
      !
      INTEGER                  ::   ji, jj, jk   ! dummy loop arguments
      REAL(wp), DIMENSION(ntsj-(nn_hls):ntej+(nn_hls)) :: p_fval  ! function value
      !!--------------------------------------------------------------------
      !
      p_fval(:) = 0._wp
   ...
   FUNCTION ptr_sj_2d( pvflx, pmsk )   RESULT ( p_fval )
      ...
      !! ** Action  : - p_fval: i-k-mean poleward flux of pvflx
      !!----------------------------------------------------------------------
      REAL(wp) , INTENT(in), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))     ::   pvflx  ! mask flux array at V-point
      REAL(wp) , INTENT(in), DIMENSION(jpi,jpj) ::   pmsk   ! Optional 2D basin mask
      !
   ...
   FUNCTION ptr_sjk( pta, pmsk )   RESULT ( p_fval )
      ...
      !!
      IMPLICIT none
      REAL(wp) , INTENT(in), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) ::   pta    ! mask flux array at V-point
      REAL(wp) , INTENT(in), DIMENSION(jpi,jpj) ::   pmsk   ! Optional 2D basin mask
      !!
      INTEGER                           :: ji, jj, jk ! dummy loop arguments
      REAL(wp), DIMENSION(ntsj-(nn_hls):ntej+(nn_hls),jpk) :: p_fval     ! return function value
      !!--------------------------------------------------------------------
      !
      p_fval(:,:) = 0._wp
      !
