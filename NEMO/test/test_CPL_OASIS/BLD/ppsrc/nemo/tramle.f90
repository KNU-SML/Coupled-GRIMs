...
CONTAINS
   ...
   SUBROUTINE tra_mle_trp( kt, kit000, pu, pv, pw, cdtype, Kmm )
      ...
      REAL(dp), DIMENSION(jpi,jpj,jpk), INTENT(inout) ::   pv         ! out: same 3  transport components
      ...
      REAL(wp) ::   zcvw, zmvw          !   -      -
      INTEGER , DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))     :: inml_mle
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))     :: zpsim_u, zpsim_v, zmld, zbm, zhu, zhv, zn2, zLf_NH, zLf_MH
      ...
      ELSE !do not use osn_mle
         ...
         IF ( nla10 > 0 ) THEN                            ! avoid case where first level is thicker than 10m
           DO jk =  jpkm1,  nlb10,  -1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)        ! from the bottom to nlb10 (10m)
         ...
         zbm (:,:) = 0._wp
         zn2 (:,:) = 0._wp
         DO jk =  1,  ikmax  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)                    ! MLD and mean buoyancy and N2 over the mixed layer
    ...
    zpsi_vw(:,:,:) = 0._wp
    !
      DO jk =  2,  ikmax  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls-1)                ! start from 2 : surface value = 0
      ...
      DO jk = 1, ikmax
         DO jj = ntsj-(  nn_hls-( nn_hls+ nn_hls-1 )*nthb), ntej+(  nn_hls-1 -( nn_hls-1 + nn_hls)*ntht) ; DO ji = ntsi-( nn_hls-( nn_hls+ nn_hls-1)*nthl), ntei+(  nn_hls-1-( nn_hls-1+ nn_hls)*nthr)
      ...
      IF( cdtype == 'TRA') THEN              !==  outputs  ==!
         ...
         DO jk =  1,  ikmax+1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namtra_mle in configuration namelist' )
      ...
      IF( ln_mle ) THEN                ! MLE initialisation
         ...
         IF( nn_mle == 0 ) THEN           ! MLE array allocation & initialisation
            ...
            DO jj = ntsj-( 0), ntej+( 1 ) ; DO ji = ntsi-( 0), ntei+( 1)                      ! "coriolis+ time^-1" at u- & v-points
