...
CONTAINS
   ...
   SUBROUTINE ice_update_tau( kt, pu_oce, pv_oce )
      ...
      IF( MOD( kt-1, nn_fsbc ) == 0 ) THEN     !==  Ice time-step only  ==!   (i.e. surface module time-step)
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)                          !* update the modulus of stress at ocean surface (T-point)
      ...
      ENDIF
      !
      DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)                             !* update the stress WITHOUT an ice-ocean rotation angle
