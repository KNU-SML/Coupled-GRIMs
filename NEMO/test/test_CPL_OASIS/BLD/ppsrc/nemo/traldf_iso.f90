...
CONTAINS
  ...
  SUBROUTINE tra_ldf_iso_t( kt, Kmm, kit000, cdtype, pahu, pahv, ktah,                    &
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))     ::   zdkt, zdk1t, z2d
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) ::   zdit, zdjt, zftu, zftv, ztfw
      !!----------------------------------------------------------------------
      !
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij), ntej+(  iij-1) ; DO ji = ntsi-( iij), ntei+(  iij-1)
         ...
         IF( ln_zps ) THEN      ! botton and surface ocean correction of the horizontal gradient
            DO jj = ntsj-( iij), ntej+( iij-1 ) ; DO ji = ntsi-( iij), ntei+( iij-1)            ! bottom correction (partial bottom cell)
         ...
         DO jk = 1, jpkm1                                 ! Horizontal slab
            ...
            END DO   ;   END DO
            !
            DO jj = ntsj-( iij), ntej+( iij-1 ) ; DO ji = ntsi-( iij), ntei+( iij-1)           !==  Horizontal fluxes
            ...
            END DO   ;   END DO
            !
            DO jj = ntsj-( iij-1), ntej+( iij-1 ) ; DO ji = ntsi-( iij-1), ntei+( iij-1)           !== horizontal divergence and add to pta
               ! round brackets added to fix the order of floating point operations
               ! needed to ensure halo 1 - halo 2 compatibility
         ...
         DO jk =  2,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)    ! interior (2=<jk=<jpk-1)
            !
            zmsku = wmask(ji,jj,jk) / MAX(   umask(ji  ,jj,jk-1) + umask(ji-1,jj,jk)          &
               &                           + umask(ji-1,jj,jk-1) + umask(ji  ,jj,jk) , 1._wp  )
            zmskv = wmask(ji,jj,jk) / MAX(   vmask(ji,jj  ,jk-1) + vmask(ji,jj-1,jk)          &
