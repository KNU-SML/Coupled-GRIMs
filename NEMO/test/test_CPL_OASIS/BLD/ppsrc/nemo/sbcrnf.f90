   ...
   SUBROUTINE sbc_rnf_div( phdivn, Kmm )
      ...
      IF( ln_rnf_depth .OR. ln_rnf_depth_ini ) THEN      !==   runoff distributed over several levels   ==!
         ...
         ELSE                    !* variable volume case
            DO jj = ntsj-(  nn_hls-( nn_hls+ nn_hls )*nthb), ntej+(  nn_hls -( nn_hls + nn_hls)*ntht) ; DO ji = ntsi-( nn_hls-( nn_hls+ nn_hls)*nthl), ntei+(  nn_hls-( nn_hls+ nn_hls)*nthr)         ! update the depth over which runoffs are distributed
               h_rnf(ji,jj) = 0._wp
               ...
               END DO
            END DO   ;   END DO
            DO jj = ntsj-(  nn_hls-1-( nn_hls-1+ nn_hls )*nthb), ntej+(  nn_hls -( nn_hls + nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+ nn_hls)*nthl), ntei+(  nn_hls-( nn_hls+ nn_hls-1)*nthr)         ! apply the runoff input flow
               DO jk = 1, nk_rnf(ji,jj)
                  phdivn(ji,jj,jk) = phdivn(ji,jj,jk) - ( rnf(ji,jj) + rnf_b(ji,jj) ) * zfact * r1_rho0 / h_rnf(ji,jj)
               END DO
            END DO   ;   END DO
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_rnf in configuration namelist' )
      ...
      IF( ln_rnf_depth ) THEN                    ! depth of runoffs set from a file
         ...
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)
            ...
            ENDIF
         END DO   ;   END DO
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)                           ! set the associated depth
            h_rnf(ji,jj) = 0._wp
            DO jk = 1, nk_rnf(ji,jj)
      ...
      ELSE IF( ln_rnf_depth_ini ) THEN           ! runoffs applied at the surface
         ...
         !
         DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)                ! take in account min depth of ocean rn_hmin
