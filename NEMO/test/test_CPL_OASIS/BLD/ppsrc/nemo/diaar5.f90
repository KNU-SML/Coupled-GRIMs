...
!!gm
      ...
      IF( iom_use( 'tnpeo' )) THEN
        ...
        ! work is not being done against stratification
         ...
         IF( ln_zdfddm ) THEN
            ...
            END DO   ;   END DO   ;   END DO
          ELSE
            DO jk =  1,  jpk  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)
   ...
   SUBROUTINE dia_ar5_hst( ktra, cptr, puflx, pvflx )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk)    , INTENT(in)   :: puflx  ! u-flux of advection/diffusion
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk)    , INTENT(in)   :: pvflx  ! v-flux of advection/diffusion
      !
      INTEGER    ::  ji, jj, jk
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))  :: z2d
      !!----------------------------------------------------------------------
   ...
   SUBROUTINE dia_ar5_init
      ...
      IF( l_ar5 ) THEN
         ...
         zvol0 (:,:) = 0._wp
         thick0(:,:) = 0._wp
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! interpolation of salinity at the last ocean level (i.e. the partial step)
         ...
         IF( iom_use( 'sshthster' ) ) THEN
            ...
            IF( ln_zps ) THEN               ! z-coord. partial steps
               DO jj = ntsj-( 1), ntej+( 1 ) ; DO ji = ntsi-( 1), ntei+( 1)          ! interpolation of salinity at the last ocean level (i.e. the partial step)
                  ik = mbkt(ji,jj)
                  IF( ik > 1 ) THEN
