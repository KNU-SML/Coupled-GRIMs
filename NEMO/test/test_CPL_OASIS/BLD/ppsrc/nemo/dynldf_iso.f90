...
CONTAINS
   ...
   SUBROUTINE dyn_ldf_iso( kt, Kbb, Kmm, puu, pvv, Krhs )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),jpk) ::   zfuw, zdiu, zdju, zdj1u  !  -      -
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),jpk) ::   zfvw, zdiv, zdjv, zdj1v  !  -      -
      !!----------------------------------------------------------------------
      !
      ...
      IF( .NOT. l_istiled .OR. ntile == 1 )  THEN                       ! Do only on the first tile
         IF( kt == nit000 ) THEN
            ...
            !
            DO jj = ntsj-(  0-( 0+ 0 )*nthb), ntej+(  0 -( 0 + 0)*ntht) ; DO ji = ntsi-( 0-( 0+ 0)*nthl), ntei+(  0-( 0+ 0)*nthr)
               akzu(ji,jj,1)   = 0._wp
...
!!gm bug is dyn_ldf_iso called before tra_ldf_iso ....   <<<<<===== TO BE CHECKED
      ...
      IF( ln_dynldf_hor .AND. ln_traldf_iso ) THEN
         !
         DO jk =  1,  jpk  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)      ! set the slopes of iso-level
