...
CONTAINS
   ...
   SUBROUTINE dia_hth( kt, Kmm )
      ...
      IF( l_hth ) THEN
         ...
         IF( iom_use( 'mlddzt' ) .OR. iom_use( 'mldr0_3' ) .OR. iom_use( 'mldr0_1' ) ) THEN
            ...
            ! MLD: rho = rho(1) + zrho1                                     !
            ! ------------------------------------------------------------- !
            DO jk =  jpkm1,  2,  -1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! loop from bottom to 2
               !
         ...
         IF(  iom_use( 'mld_dt02' ) .OR. iom_use( 'topthdep' ) .OR. iom_use( 'mldr10_3' ) .OR.  &    
            ...
            ! ------------------------------------------------------------- !
            DO jk =  jpkm1,  nlb10,  -1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! loop from bottom to nlb10
               !
   ...
   SUBROUTINE dia_hth_dep( Kmm, ptem, pdept )
      ...
      ! --------------------------------------- !
      iktem(:,:) = 1
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! beware temperature is not always decreasing with depth => loop from top to bottom
