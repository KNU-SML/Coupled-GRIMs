...
CONTAINS
   ...
   SUBROUTINE dyn_zad ( kt, Kmm, puu, pvv, Krhs )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))     ::   zww
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) ::   zwuw, zwvw
      ...
      DO jk = 2, jpkm1                ! Vertical momentum advection at level w and u- and v- vertical
         ...
         ELSE
            ...
            END DO   ;   END DO
         ENDIF
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)              ! vertical momentum advection at w-point
      ...
      END DO   ;   END DO
      !
      DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)   ! Vertical momentum advection at u- and v-points
