...
CONTAINS
   ...
   SUBROUTINE ldf_slp( kt, prd, pn2, Kbb, Kmm )
      ...
      zwz(:,:,:) = 0._wp
      !
      DO jk =  1,  jpk  ; DO jj = ntsj-(  1), ntej+(  0) ; DO ji = ntsi-( 1), ntei+(  0)   !==   i- & j-gradient of density   ==!
      ...
      DO jk =  2,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)        !* Slopes at u and v points
...
!!gm end modif
      ...
      DO jk = 2, jpkm1
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)                                 ! rows jj=2 and =jpjm1 only
...
!!gm end modif
   ...
   SUBROUTINE ldf_slp_triad ( kt, Kbb, Kmm )
      ...
      DO jl = 0, 1                            !==  unmasked before density i- j-, k-gradients  ==!
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls-1)        ! done each pair of triad ! NB: not masked ==>  a minimum value is set
      ...
      DO kp = 0, 1                            !==  unmasked before density i- j-, k-gradients  ==!
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)      ! done each pair of triad ! NB: not masked ==>  a minimum value is set
      ...
      END DO
      !
      DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)                   !== Reciprocal depth of the w-point below ML base  ==!
   ...
   SUBROUTINE ldf_slp_mxl( prd, pn2, p_gru, p_grv, p_dzr, Kmm )
      ...
      DO jk =  1,  jpk  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)                  ! =1 inside the mixed layer, =0 otherwise
         ik = nmln(ji,jj) - 1
