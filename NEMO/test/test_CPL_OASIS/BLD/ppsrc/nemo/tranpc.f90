...
CONTAINS
   ...
   SUBROUTINE tra_npc( kt, Kmm, Krhs, pts, Kaa )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk,jpts)   ::   zab              ! alpha and beta
      ...
      IF( MOD( kt, nn_npc ) == 0 ) THEN
         ...
         !
         DO jj = ntsj-(  0-( 0+ 0 )*nthb), ntej+(  0 -( 0 + 0)*ntht) ; DO ji = ntsi-( 0-( 0+ 0)*nthl), ntei+(  0-( 0+ 0)*nthr)                        ! interior column only
            !
