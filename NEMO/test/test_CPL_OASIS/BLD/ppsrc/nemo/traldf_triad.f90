...
CONTAINS
  ...
  SUBROUTINE tra_ldf_triad_t( kt, Kmm, kit000, cdtype, pahu, pahv, ktah,                   &
      ...
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktah+1:,(ntsj-nn_hls-1)*ktah+1:,   :)     , INTENT(in   ) ::   pahu, pahv ! eddy diffusivity at u- and v-points  [m2/s]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktg+1:,(ntsj-nn_hls-1)*ktg+1:,        :), INTENT(in   ) ::   pgu , pgv  ! tracer gradient at pstep levels
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktgi+1:,(ntsj-nn_hls-1)*ktgi+1:,       :), INTENT(in   ) ::   pgui, pgvi ! tracer gradient at top   levels
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktt+1:,(ntsj-nn_hls-1)*ktt+1:,    :,:), INTENT(in   ) ::   pt         ! tracer (kpass=1) or laplacian of tracer (kpass=2)
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktt2+1:,(ntsj-nn_hls-1)*ktt2+1:,   :,:), INTENT(in   ) ::   pt2        ! tracer (only used in kpass=2)
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktt_rhs+1:,(ntsj-nn_hls-1)*ktt_rhs+1:,:,:), INTENT(inout) ::   pt_rhs     ! tracer trend
      !
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),0:1) ::   zdkt3d                                           ! vertical tracer gradient at 2 levels
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls)    ) ::   z2d                                              ! 2D workspace
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) ::   zdit, zdjt, zftu, zftv, ztfw, zpsi_uw, zpsi_vw   ! 3D     -
      !!----------------------------------------------------------------------
      !
...
!!gm  this should probably be done outside the jn loop
         ...
         zdjt(:,:,:) = 0._wp
         !
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij), ntej+(  iij-1) ; DO ji = ntsi-( iij), ntei+(  iij-1)    !==  before lateral T & S gradients at T-level jk  ==!
         ...
         IF( ln_zps .AND. l_grad_zps ) THEN    ! partial steps: correction at top/bottom ocean level
            DO jj = ntsj-( iij), ntej+( iij-1 ) ; DO ji = ntsi-( iij), ntei+( iij-1)                    ! bottom level
               zdit(ji,jj,mbku(ji,jj)) = pgu(ji,jj,jn)
               zdjt(ji,jj,mbkv(ji,jj)) = pgv(ji,jj,jn)
            END DO   ;   END DO
            IF( ln_isfcav ) THEN                   ! top level (ocean cavities only)
               DO jj = ntsj-( iij), ntej+( iij-1 ) ; DO ji = ntsi-( iij), ntei+( iij-1)
         ...
         DO jk = 1, jpkm1
            ...
            DO jj = ntsj-( iij), ntej+( iij ) ; DO ji = ntsi-( iij), ntei+( iij)
         ...
         IF( ln_traldf_lap ) THEN               ! laplacian case: eddy coef = ah_wslp2 - akz
            DO jk =  2,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)
         ...
         ENDIF
         !
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  iij-1), ntej+(  iij-1) ; DO ji = ntsi-( iij-1), ntei+(  iij-1)      !==  Divergence of vertical fluxes added to pta  ==!
            pt_rhs(ji,jj,jk,jn) = pt_rhs(ji,jj,jk,jn)    &
