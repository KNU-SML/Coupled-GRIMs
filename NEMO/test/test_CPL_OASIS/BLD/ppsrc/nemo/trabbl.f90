...
CONTAINS
   ...
   SUBROUTINE tra_bbl_dif( pt, pt_rhs, kjpt, Kmm )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls)) ::   zptb   ! workspace
      !!----------------------------------------------------------------------
      !
      DO jn = 1, kjpt                                     ! tracer loop
         ...
         END DO   ;   END DO
         !
         DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)                               ! Compute the trend
            ik = mbkt(ji,jj)                            ! bottom T-level index
   ...
   SUBROUTINE tra_bbl_adv( pt, pt_rhs, kjpt, Kmm )
      ...
      DO jn = 1, kjpt                                            ! tracer loop
         !                                                       ! ===========
         DO jj = ntsj-(  1-( 1+ 0 )*nthb), ntej+(  0 -( 0 + 1)*ntht) ; DO ji = ntsi-( 1-( 1+ 0)*nthl), ntei+(  0-( 0+ 1)*nthr)            ! CAUTION start from i=1 to update i=2 when cyclic east-west
   ...
   SUBROUTINE bbl( kt, kit000, Kbb, Kmm )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpts)   :: zts, zab         ! 3D workspace
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls))        :: zub, zvb, zdep   ! 2D workspace
      !!----------------------------------------------------------------------
      !
      ...
      IF( nn_bbl_ldf == 1 ) THEN          !   diffusive bbl   !
         !                                !-------------------!
         DO jj = ntsj-(  1-( 1+ 0 )*nthb), ntej+(  0 -( 0 + 1)*ntht) ; DO ji = ntsi-( 1-( 1+ 0)*nthl), ntei+(  0-( 0+ 1)*nthr)                   ! (criteria for non zero flux: grad(rho).grad(h) < 0 )
            !                                                   ! i-direction
      ...
      IF( nn_bbl_adv /= 0 ) THEN          !   advective bbl   !
         ...
         CASE( 1 )                                   != use of upper velocity
            DO jj = ntsj-(  1-( 1+ 0 )*nthb), ntej+(  0 -( 0 + 1)*ntht) ; DO ji = ntsi-( 1-( 1+ 0)*nthl), ntei+(  0-( 0+ 1)*nthr)                              ! criteria: grad(rho).grad(h)<0  and grad(rho).grad(h)<0
               !                                                  ! i-direction
               za = zab(ji+1,jj,jp_tem) + zab(ji,jj,jp_tem)               ! 2*(alpha,beta) at u-point
               zb = zab(ji+1,jj,jp_sal) + zab(ji,jj,jp_sal)
               !                                                          ! 2*masked bottom density gradient
         ...
         CASE( 2 )                                 != bbl velocity = F( delta rho )
            ...
            DO jj = ntsj-(  1-( 1+ 0 )*nthb), ntej+(  0 -( 0 + 1)*ntht) ; DO ji = ntsi-( 1-( 1+ 0)*nthl), ntei+(  0-( 0+ 1)*nthr)                         ! criteria: rho_up > rho_down
               !                                                  ! i-direction
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nambbl in configuration namelist' )
      ...
      DO jj = ntsj-( 1), ntej+( 0 ) ; DO ji = ntsi-( 1), ntei+( 0)                 ! (the "shelf" bottom k-indices are mbku and mbkv)
      ...
      END DO   ;   END DO
      !
      DO jj = ntsj-( 1), ntej+( 0 ) ; DO ji = ntsi-( 1), ntei+( 0)           !* bbl thickness at u- (v-) point; minimum of top & bottom e3u_0 (e3v_0)
