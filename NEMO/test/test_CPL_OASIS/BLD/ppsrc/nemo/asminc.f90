...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_asminc in configuration namelist' )
   ...
   SUBROUTINE tra_asm_inc( kt, Kbb, Kmm, pts, Krhs )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) :: fzptnz ! 3d freezing point values
      !!----------------------------------------------------------------------
      !
            ...
            DO jk = 1, jpkm1
               IF (ln_temnofreeze) THEN
                  ...
                  WHERE(t_bkginc(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk) > 0.0_wp .OR. &
                     &   pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_tem,Kmm) + pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_tem,Krhs) + t_bkginc(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk) * wgtiau(it) > fzptnz(:,:,jk) )
                     pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_tem,Krhs) = pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_tem,Krhs) + t_bkginc(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk) * zincwgt
                  END WHERE
               ELSE
                  DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
               ...
               IF (ln_salfix) THEN
                  ...
                  WHERE(s_bkginc(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk) > 0.0_wp .OR. &
                     &   pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_sal,Kmm) + pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_sal,Krhs) + s_bkginc(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk) * wgtiau(it) > salfixmin )
                     pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_sal,Krhs) = pts(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk,jp_sal,Krhs) + s_bkginc(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jk) * zincwgt
                  END WHERE
               ELSE
                  DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)
...
!!gm why not e3u(:,:,:,Kbb), e3v(:,:,:,Kbb), gdept(:,:,:,Kbb) ????
   ...
   SUBROUTINE seaice_asm_inc( kt, kindic )
      ...
      INTEGER  ::   it
      REAL(wp) ::   zincwgt   ! IAU weight for current time step
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls)) ::   zofrld, zohicif, zseaicendg, zhicifinc
      ...
      IF ( ln_asmiau ) THEN         ! Incremental Analysis Updating
         ...
         IF ( ( kt >= nitiaustr_r ).AND.( kt <= nitiaufin_r ) ) THEN
            ...
            WHERE( zseaicendg(:,:) > 0.0_wp .AND. hm_i(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0)) < zhicifmin )
               zhicifinc(:,:) = (zhicifmin - hm_i(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0))) * zincwgt
            ELSEWHERE
               zhicifinc(:,:) = 0.0_wp
            END WHERE
            !
      ...
      ELSEIF ( ln_asmdin ) THEN     ! Direct Initialization
         ...
         IF ( kt == nitdin_r ) THEN
            ...
            ! Nudge sea ice depth to bring it up to a required minimum depth
            WHERE( zseaicendg(:,:) > 0.0_wp .AND. hm_i(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0)) < zhicifmin )
               zhicifinc(:,:) = zhicifmin - hm_i(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0))
            ELSEWHERE
               zhicifinc(:,:) = 0.0_wp
            END WHERE
            !
