...
CONTAINS
   ...
   SUBROUTINE dom_vvl_zgr(Kbb, Kmm, Kaa)
      ...
      gdepw(:,:,1,Kbb) = 0.0_wp
      DO jk =  2,  jpk  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)                     ! vertical sum
...
!!gm  case |gphi| >= 6 degrees is useless   initialized just above by default
   ...
   SUBROUTINE dom_vvl_sf_nxt( kt, Kbb, Kmm, Kaa, kcall )
      ...
      IF( (ln_vvl_ztilde .OR. ln_vvl_layer) .AND. ll_do_bclinic ) THEN   ! z_tilde or layer coordinate !
         ...
         zwu(:,:) = 0._wp
         zwv(:,:) = 0._wp
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls-1) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls-1)       ! a - first derivative: diffusive fluxes
         ...
         END DO   ;   END DO   ;   END DO
         DO jj = ntsj-( nn_hls), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls-1)                 ! b - correction for last oceanic u-v points
            un_td(ji,jj,mbku(ji,jj)) = un_td(ji,jj,mbku(ji,jj)) - zwu(ji,jj)
            vn_td(ji,jj,mbkv(ji,jj)) = vn_td(ji,jj,mbkv(ji,jj)) - zwv(ji,jj)
         END DO   ;   END DO
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)   ! c - second derivative: divergence of diffusive fluxes
...
!!gm BUG ?  don't understand the use of umask_i here .....
   ...
   SUBROUTINE dom_vvl_sf_update( kt, Kbb, Kmm, Kaa )
      ...
      DO jk =  2,  jpk  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)
