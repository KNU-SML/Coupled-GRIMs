...
CONTAINS
   ...
   SUBROUTINE eos_insitu_t( pts, ktts, prd, ktrd, pdep, ktdep )
      ...
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktts+1:,(ntsj-nn_hls-1)*ktts+1: ,:,:), INTENT(in   ) ::   pts   ! 1 : potential temperature  [Celsius]
      !                                                                  ! 2 : salinity               [psu]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktrd+1:,(ntsj-nn_hls-1)*ktrd+1: ,:     ), INTENT(  out) ::   prd   ! in situ density            [-]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktdep+1:,(ntsj-nn_hls-1)*ktdep+1:,:     ), INTENT(in   ) ::   pdep  ! depth                      [m]
      !
   ...
   SUBROUTINE eos_insitu_pot_t( pts, ktts, prd, ktrd, prhop, ktrhop, pdep, ktdep )
      ...
      !                                                                    ! 2 : salinity               [psu]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktrd+1:,(ntsj-nn_hls-1)*ktrd+1:  ,:     ), INTENT(  out) ::   prd    ! in situ density            [-]
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktrhop+1:,(ntsj-nn_hls-1)*ktrhop+1:,:     ), INTENT(  out) ::   prhop  ! potential density (surface referenced)
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktdep+1:,(ntsj-nn_hls-1)*ktdep+1: ,:     ), INTENT(in   ) ::   pdep   ! depth                      [m]
      !
   ...
   SUBROUTINE eos_insitu_2d_t( pts, ktts, pdep, ktdep, prd, ktrd )
      ...
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktts+1:,(ntsj-nn_hls-1)*ktts+1:,:), INTENT(in   ) ::   pts   ! 1 : potential temperature  [Celsius]
      !                                                             ! 2 : salinity               [psu]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktdep+1:,(ntsj-nn_hls-1)*ktdep+1:    ), INTENT(in   ) ::   pdep  ! depth                      [m]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktrd+1:,(ntsj-nn_hls-1)*ktrd+1:     ), INTENT(  out) ::   prd   ! in situ density
      !
   ...
   SUBROUTINE eos_insitu_pot_2d_t( pts, ktts, prhop, ktrhop )
      ...
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktts+1:,(ntsj-nn_hls-1)*ktts+1:,:), INTENT(in   ) ::   pts    ! 1 : potential temperature  [Celsius]
      !                                                                ! 2 : salinity               [psu]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktrhop+1:,(ntsj-nn_hls-1)*ktrhop+1:   ), INTENT(  out) ::   prhop  ! potential density (surface referenced)
      !
      INTEGER  ::   ji, jj, jk, jsmp             ! dummy loop indices
      INTEGER  ::   jdof
   ...
   SUBROUTINE rab_3d_t( pts, ktts, pab, ktab, Kmm )
      ...
      REAL(dp), DIMENSION((ntsi-nn_hls-1)*ktts+1:,(ntsj-nn_hls-1)*ktts+1:,:,:), INTENT(in   ) ::   pts   ! pot. temperature & salinity
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktab+1:,(ntsj-nn_hls-1)*ktab+1:,:,:), INTENT(  out) ::   pab   ! thermal/haline expansion ratio
      !
      INTEGER  ::   ji, jj, jk                ! dummy loop indices
   ...
   SUBROUTINE rab_2d_t( pts, ktts, pdep, ktdep, pab, ktab, Kmm )
      ...
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktts+1:,(ntsj-nn_hls-1)*ktts+1:,:), INTENT(in   ) ::   pts    ! pot. temperature & salinity
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktdep+1:,(ntsj-nn_hls-1)*ktdep+1:    ), INTENT(in   ) ::   pdep   ! depth                  [m]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktab+1:,(ntsj-nn_hls-1)*ktab+1:,:), INTENT(  out) ::   pab    ! thermal/haline expansion ratio
      !
      INTEGER  ::   ji, jj, jk                ! dummy loop indices
   ...
   SUBROUTINE bn2_t( pts, pab, ktab, pn2, ktn2, Kmm )
      ...
      REAL(dp), DIMENSION(jpi,jpj,  jpk,jpts), INTENT(in   ) ::  pts   ! pot. temperature and salinity   [Celsius,psu]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktab+1:,(ntsj-nn_hls-1)*ktab+1:,:,:), INTENT(in   ) ::  pab   ! thermal/haline expansion coef.  [Celsius-1,psu-1]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*ktn2+1:,(ntsj-nn_hls-1)*ktn2+1:,:     ), INTENT(  out) ::  pn2   ! Brunt-Vaisala frequency squared [1/s^2]
      !
      INTEGER  ::   ji, jj, jk      ! dummy loop indices
      ...
      !
      DO jk =  2,  jpkm1  ; DO jj = ntsj-(  nn_hls), ntej+(  nn_hls) ; DO ji = ntsi-( nn_hls), ntei+(  nn_hls)      ! interior points only (2=< jk =< jpkm1 ); surface and bottom value set to zero one for all in istate.F90
   ...
   SUBROUTINE  eos_fzp_2d_t( psal, ptf, kttf, pdep )
      ...
      REAL(wp), DIMENSION(jpi,jpj)  , INTENT(in   )           ::   psal   ! salinity   [psu]
      REAL(dp), DIMENSION(jpi,jpj)  , INTENT(in   ), OPTIONAL ::   pdep   ! depth      [m]
      REAL(wp), DIMENSION((ntsi-nn_hls-1)*kttf+1:,(ntsj-nn_hls-1)*kttf+1:), INTENT(out  )           ::   ptf    ! freezing temperature [Celsius]
      !
      INTEGER  ::   ji, jj          ! dummy loop indices
