...
CONTAINS
   ...
   SUBROUTINE ice_dyn_rhg_evp( kt, Kmm, pstress1_i, pstress2_i, pstress12_i, pshear_i, pdivu_i, pdelta_i )
      ...
      DO jter = 1 , nn_nevp                           !    loop over jter    !
         ...
         !
         DO jj = ntsj-( nn_hls-1), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls)   ! loop ends at jpi,jpj so that no lbc_lnk are needed for zs1 and zs2
         ...
         IF( ll_advups .AND. ln_str_H79 ) THEN
            ...
            !
            DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)    ! strength
      ...
      DO jj = ntsj-( 0), ntej+( 0 ) ; DO ji = ntsi-( 0), ntei+( 0)   ! no vector loop
      ...
      DO jl = 1, jpl
         ...
         ELSE                              !** alternate directions **!
            ...
            IF( MOD(jter,2) == 1 ) THEN   !==  odd ice time step:  adv_x then adv_y  ==!
               !
               DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls-1)       !-- flux in x-direction
                  zfu_ups(ji,jj) = MAX( pu(ji,jj)*e2u(ji,jj), 0._wp ) * pt(ji  ,jj,jl) + &
                     &             MIN( pu(ji,jj)*e2u(ji,jj), 0._wp ) * pt(ji+1,jj,jl)
               END DO   ;   END DO
               !
               DO jj = ntsj-( nn_hls), ntej+( nn_hls ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls-1)     !-- first guess of tracer from u-flux
                  ztra       = - ( zfu_ups(ji,jj) - zfu_ups(ji-1,jj) )
                  zpt(ji,jj) =   ( pt(ji,jj,jl) + ztra * pdt * r1_e1e2t(ji,jj) ) * tmask(ji,jj,1)
               END DO   ;   END DO
               !
               DO jj = ntsj-( nn_hls), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls-1), ntei+( nn_hls-1)   !-- flux in y-direction
            ...
            ELSE                          !==  even ice time step:  adv_y then adv_x  ==!
               !
               DO jj = ntsj-( nn_hls), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)       !-- flux in y-direction
                  zfv_ups(ji,jj) = MAX( pv(ji,jj)*e1v(ji,jj), 0._wp ) * pt(ji,jj  ,jl) + &
                     &             MIN( pv(ji,jj)*e1v(ji,jj), 0._wp ) * pt(ji,jj+1,jl)
               END DO   ;   END DO
               !
               DO jj = ntsj-( nn_hls-1), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls)     !-- first guess of tracer from v-flux
                  ztra       = - ( zfv_ups(ji,jj) - zfv_ups(ji,jj-1) )
                  zpt(ji,jj) =   ( pt(ji,jj,jl) + ztra * pdt * r1_e1e2t(ji,jj) ) * tmask(ji,jj,1)
               END DO   ;   END DO
               !
               DO jj = ntsj-( nn_hls-1), ntej+( nn_hls-1 ) ; DO ji = ntsi-( nn_hls), ntei+( nn_hls-1)   !-- flux in x-direction
