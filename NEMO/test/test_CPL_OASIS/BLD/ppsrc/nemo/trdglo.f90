...
!!----------------------------------------------------------------------
   ...
   !! $Id: trdglo.F90 13497 2020-09-21 12:37:46Z techene $
   !! Software governed by the CeCILL license (see ./LICENSE)
   !!----------------------------------------------------------------------
CONTAINS
      ...
      IF( MOD(kt,nn_trd) == 0 .OR. kt == nit000 .OR. kt == nitend ) THEN
         !
      	...
      	CASE( 'TRA' )          !==  Tracers (T & S)  ==!
      	   DO jk =  1,  jpkm1  ; DO jj = ntsj-(  1), ntej+(  1) ; DO ji = ntsi-( 1), ntei+(  1)   ! global sum of mask volume trend and trend*T (including interior mask)
...
!!gm end
   ...
   SUBROUTINE glo_dyn_wri( kt, Kmm )
      ...
      IF( MOD( kt, nn_trd ) == 0 .OR. kt == nit000 .OR. kt == nitend ) THEN
         ...
         DO jk =  1,  jpkm1  ; DO jj = ntsj-(  0), ntej+(  0) ; DO ji = ntsi-( 0), ntei+(  0)    ! Density flux divergence at t-point
