...
!!gm
   ...
   SUBROUTINE zdf_phy( kt, Kbb, Kmm, Krhs )
      ...
      REAL(wp), DIMENSION(ntsi-(nn_hls):ntei+(nn_hls),ntsj-(nn_hls):ntej+(nn_hls),jpk) ::   zsh2   ! shear production
      !! ---------------------------------------------------------------------
      !
...
!!gm         avm(2:jpim1,2:jpjm1,1:jpkm1) = rn_avm0 * wmask(2:jpim1,2:jpjm1,1:jpkm1)
      ...
      IF( ln_rnf_mouth ) THEN                   !* increase diffusivity at rivers mouths
         DO jk =  2,  nkrnf  ; DO jj = ntsj-(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*nthb), ntej+(   nn_hls-1-(  nn_hls-1+  nn_hls-1)*ntht) ; DO ji = ntsi-( nn_hls-1-( nn_hls-1+  nn_hls-1)*nthl), ntei+(   nn_hls-1-(  nn_hls-1+ nn_hls-1)*nthr)
      ...
      IF( iom_use('avt_k') .OR. iom_use('avm_k') .OR. iom_use('eshear_k') .OR. iom_use('estrat_k') ) THEN
         IF( l_zdfsh2 ) THEN
            ...
            CALL iom_put( 'eshear_k',   zsh2(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),:) * wmask(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),:) )
