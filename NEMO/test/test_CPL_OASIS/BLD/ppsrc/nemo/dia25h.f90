...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_dia25h in configuration namelist' )
      ...
      ! ------------------- !
      !                                ! ocean arrays
      ALLOCATE( tn_25h (ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), sn_25h (ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), sshn_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0))  ,     &
         &      un_25h (ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), vn_25h (ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), wn_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk),     &
         &      avt_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), avm_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk),                      STAT=ierror )
      IF( ierror > 0 ) THEN
         CALL ctl_stop( 'dia_25h: unable to allocate ocean arrays' )   ;   RETURN
      ENDIF
      IF( ln_zdftke ) THEN             ! TKE physics
         ALLOCATE( en_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), STAT=ierror )
         IF( ierror > 0 ) THEN
            CALL ctl_stop( 'dia_25h: unable to allocate en_25h' )   ;   RETURN
         ENDIF
      ENDIF
      IF( ln_zdfgls ) THEN             ! GLS physics
         ALLOCATE( en_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), rmxln_25h(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk), STAT=ierror )
         IF( ierror > 0 ) THEN
   ...
   SUBROUTINE dia_25h( kt, Kmm )  
      ...
      REAL(wp), DIMENSION(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0)    )  ::   zw2d, un_dm, vn_dm                ! workspace
      REAL(wp), DIMENSION(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),jpk)  ::   zw3d                              ! workspace
      REAL(wp), DIMENSION(ntsi-(0):ntei+(0),ntsj-(0):ntej+(0),3)    ::   zwtmb                             ! workspace
      !!----------------------------------------------------------------------
