...
!-------------------------------------------------------------------------------
   subroutine nsas2d(delt,delx,del,prsl,prsi,prslk,zl,                         &
            ...
            its,ite, jts,jte, kts,kte)
#else    
   subroutine phys_cps_ksas(delt,delx,del,prsl,prsi,zl,zi,                     &
                            q2,                                                &
#ifdef SAS_CCN
                            xccn,                                              &
#endif 
...
!-------------------------------------------------------------------------------
!
! subprogram:    phys_cps_ksas      computes convective heating and moistening
...
!   15-02-26  song-you hong    negative qv generation suppressed
!   15-06-01  jongil han       gfs sas
!   15-06-07  song-you hong    scale-aware cps
#ifdef CPS_KSAS_DIUR
!   15-06-28  song-you hong    diurnal cycle with cldwrk in pbl
#endif
!   15-10-01  ji-young han     bug fix (dz to dz1)
!   16-01-01  ji-young han     enhanced entrainment at lower RH
!   16-06-01  ji-young han     moisture-based trigger threshold
!   16-09-01  ji-young han     bug fix & eliminate inconsistency
!   16-09-23  ji-young han     code clean-up
!   16-10-17  ji-young han     q : mixing ratio
!   16-11-01  ji-young han     revised pgcon & bug fix in vshear
!   17-02-23  ji-young han     revised xlamb
#ifdef SAS_CCN_OCT2017
!   17-10-20  ji-young han     revised aerosol-aware cps
#endif
!   18-03-01  ji-young han     kim sas
!   18-04-06  ji-young han     bug fix in qci & qrs
#ifdef SAS_DET_AUG2018
!   18-08-28  ji-young han     revised detrainment
#endif
...
!   19-06-11  ji-young han     optimization of c0
!   20-05-14  myung-seo koo    mpp function
#ifdef FSVP_ECMWF
!   20-05-14  myung-seo koo    upper limit in saturation vapor pressure
#endif
!
! usage:    call phys_cps_ksas(delt,delx,del,prsl,prsi,prslk,prsik,zl,          &
...
#else
!   delx     - real (ims:ime) model grid interval
#endif
!   del      - real (ims:ime,kms:kme) sigma layer thickness
!   prsl     - real (ims:ime,kms:kme) pressure values
!   prsi     - real (ims:ime,kms:kme+1) pressure values at interface level
#ifdef WRF
...
!   zi       - real (ims:ime,kms:kme+1) height above sea level at interface level
#ifdef WRF
!   rcs      - real
#endif
...
!   q2       - real (ims:ime,kms:kme,ncloud) detrained hydrometeors in kg/kg
!            - in case of the WRF --> qc2(cloud), qi2(ice)
!   q1       - real (ims:ime,kms:kme) adjusted mixing ratio in kg/kg
...
#ifndef WRF
!   dct      - real (ims:ime,kms:kme) deep convective heating rate in K/s
!   fcu      - real (ims:ime,kms:kme) updraft   mass flux in kg/m2/s
!   fcd      - real (ims:ime,kms:kme) downdraft mass flux in kg/m2/s
!   qci      - real (ims:ime,kms:kme) mixing ratio of cloud water/ice of subgrid cloud in kg/kg 
!   qrs      - real (ims:ime,kms:kme) mixing ratio of rain/snow of subgrid cloud in kg/kg
#endif
...
!
! subprograms called:
!   fsvp_pa  - function to compute saturation vapor pressure
!
! remarks: function fsvp_pa is inlined by fpp.
...
!   han et al.    (2020, wea. forecasting)
!
!-------------------------------------------------------------------------------
#ifndef WRF
   use comconsts, only : kbmaxin,kbmin,kmaxin
!   use phys_funct, only : fsvp_pa,mpp0
#ifdef DBG
   use comio,     only : iope
#endif
#endif
...
#else
   real,parameter  ::  dthk   = 25.e2
#endif
#ifdef WRF
   real,parameter  ::  cinpcrmx = 240.,cinpcrmn = 120.
#else
   real,parameter  ::  cinpcrmx = 240.e2, cinpcrmn = 120.e2
#endif
...
#ifdef LESS_THAN_10KM
   real,parameter  ::  dx10km = 10000.
#endif
#ifdef SAS_VSHEAR
   real,parameter  ::  edtmaxl = 0.9, edtmaxs = 0.9
#else
   real,parameter  ::  edtmaxl = 0.3, edtmaxs = 0.3
#endif
   real,parameter  ::  evfacts = 0.3, evfactl = 0.3
#ifndef CPS_QMIN
   real,parameter  ::  qmin10 = 1.e-10
#endif
#ifdef WRF
...
#else
   real            ::  cp_,cliq_,cvap_,g_,hvap_,rd_,rv_,fv_,cv_
   real            ::  pi_,qmin_,t0c_
#endif
...
#else
   real            ::  delt, rcs, fpvs, fpvs0
!   real            ::  delx(ims:ime)
   real :: delx
   real            ::  del(ims:ime,kms:kme),                                   &
                       prsl(ims:ime,kms:kme),                                  &
                       prsi(ims:ime,kms:kme+1),                                &
...
!
...
#else
   real            ::  rain(ims:ime),cldwrk(ims:ime)
   real            ::  hpbl(ims:ime)
#ifdef CPS_KSAS_DIUR
   real            ::  hpbl_hold(ims:ime)
#endif
   ...
   real            ::  dct(ims:ime,kms:kme),                                   &
                       fcu(ims:ime,kms:kme),fcd(ims:ime,kms:kme),              &
                       eta(ims:ime,kms:kme),etad(ims:ime,kms:kme)
   real            ::  xlamd(ims:ime)
#else
   real            ::  eta(its:ite,kts:kte),etad(its:ite,kts:kte)
   real            ::  xlamd(its:ite)
#endif
!
...
#else
   real            ::  po(its:ite,kts:kte),wu2(its:ite,kts:kte),               &
#endif
...
!
   ...
   dtmax  = max(dt2,10800.)
   mbdt   = dt2
   sigma_con = tan(0.4*pi_)/(dx5km-dx1km)                     ! 7.7 e-4 m-1
#ifdef WRF
   sigma  = (1.-1./pi_*(atan(sigma_con*(delx-dx5km))+pi_/2.)) ! 1(1km),0.1(10km)
!
...
!
   do i = its,ite
!     sigma(i) = (1.-1./pi_*(atan(sigma_con*(delx(i)-dx5km))+pi_/2.)) ! 1(1km),0.1(10km)
     sigma(i) = (1.-1./pi_*(atan(sigma_con*(delx-dx5km))+pi_/2.)) ! 1(1km),0.1(10km)
!
!     if(delx(i).lt.dx5km) then
!       sigma(i) = min(sigma(i) - 0.01684 * delx(i)/1000. + 0.0842, 1.0)
!     endif
     if(delx.lt.dx5km) then
       sigma(i) = min(sigma(i) - 0.01684 * delx/1000. + 0.0842, 1.0)
     endif
#ifdef LESS_THAN_10KM
!
!     if(delx(i).lt.dx10km) then
!       tau_dx(i) = 1. + log(dx10km/delx(i))
!     endif
     if(delx.lt.dx10km) then
       tau_dx(i) = 1. + log(dx10km/delx)
     endif
#endif
!
     cinpcr(i) = (cinpcrmn + 0.5*(cinpcrmx-cinpcrmn)) * (1.-sigma(i))
   enddo
#endif
!
!  initialize arrays
...
!yng start
!do i = its, ite
!  write(*,'(a,x,i3,x,15(f12.5,x))') "yng ksas in chk:", i, delt, delx, del(i,1), &
!                                 prsl(i,1), prsi(i,1), zl(i,1), zi(i,1), &
!                                 dot(i,1), hpbl(i), hpbl_hold(i), 1.e3*fpvs(t1(i,1))
!enddo
!yng end
...
#else
#ifdef ICE
       qeso(i,k) = 1.e3 * fpvs(to(i,k))
#else
       qeso(i,k) = 1.e3 * fpvs0(to(i,k))
#endif
       qeso(i,k) = eps * qeso(i,k) / (p(i,k) - qeso(i,k))            ! MR
       qeso(i,k) = max(qeso(i,k),qmin_)
#ifdef CPS_QMIN
       qo(i,k)   = max(qo(i,k), qmin_)
#else
       qo(i,k)   = max(qo(i,k), qmin10)
#endif
#endif
...
!
   do k = kts,kbm
     do i = its,ite
       if(flg(i).and.zl(i,k).gt.hsfc(i)) then
         ksfc(i) = k
         flg(i) = .false.
       endif
     enddo
...
#ifdef DBG
       if(iope) then
         if(i.eq.idbg.and.lat.eq.jdbg) print*,' i kb ksfc ', i,kb(i),ksfc(i)
       endif
#endif
       kb(i) = max(kb(i),ksfc(i))
     endif
   enddo
#endif
!
   ...
   do k = kts,kmax1
     do i = its,ite
       if(cnvflg(i)) then
...
#else
#ifdef ICE
         es = 1.e3 * fpvs(to(i,k+1))
#else
         es = 1.e3 * fpvs0(to(i,k+1))
#endif
         pprime = p(i,k+1) - es             ! es : saturated mixing ratio
#endif
         qs = eps * es / pprime
         dqsdp = - qs / pprime
         ...
         qeso(i,k) = 1.e3 * fpvs0(to(i,k))
#endif
         qeso(i,k) = eps * qeso(i,k) / (po(i,k) - qeso(i,k))            ! MR
         qeso(i,k) = max(qeso(i,k),qmin_)
#ifdef CPS_QMIN
         qo(i,k)   = max(qo(i,k), qmin_)
#else
...
#else
       cinpcri(i) = cinpcr(i) * frh_sum(i)/(kbcon(i)-kb(i)+1)
#endif
...
!
   do k = kts,kte1
     do i = its,ite
       xlamud(i,k) = xlamb(i,kbcon(i)) * (1.6 - (1-frh(i,k)))
     enddo
   enddo
#else
!
...
#ifdef SAS_DET_AUG2018
         ptem = 0.5*(xlamb(i,k)+xlamb(i,k+1))-0.5*(xlamud(i,k)+xlamud(i,k+1))
#else
...
!
   do i = its,ite
     if(cnvflg(i)) then
       dz = zi(i,3) - zi(i,2)
#ifdef SAS_DET_AUG2018
       ptem = 0.5*(xlamb(i,1)+xlamb(i,2))-0.5*(xlamud(i,1)+xlamud(i,2))
#else
...
#ifdef SAS_DET_AUG2018
         tem1 = 0.5 * (xlamud(i,k)+xlamud(i,k-1)) * dz
#else
...
#else
#ifdef SAS_OCT2017
     if(cnvflg(i).and.(p(i,kbcon(i)) - p(i,ktcon(i))).lt.200.e2)               &
#else
     if(cnvflg(i).and.(p(i,kbcon(i)) - p(i,ktcon(i))).lt.150.e2)               &
#endif
#endif
     cnvflg(i) = .false.
   enddo
...
#ifdef SAS_CCN
#ifdef SAS_CCN_OCT2017
         alpha1 = min((-0.7*log(xccn(i,kbcon(i))/1000000.)+24.)*0.0001,c0)
#else
         alpha1 = min((-0.7*log(xccn(i,k)/1000000.)+24.)*0.0001,c0)
#endif
#else
...
!
         if(to(i,k).gt.t0c_) then
           ...
           c0fac = alpha2*exp(beta1*(to(i,k)-t0c_))
         endif
#else
         if(to(i,k).gt.t0c_) then
           c0fac = alpha1
...
#ifdef SAS_DET_AUG2018
         tem1 = 0.5 * (xlamud(i,k)+xlamud(i,k-1)) * dz1
#else
...
#ifdef DBG
!          qci(i,k) = qlk
!          if(i.eq.1) write(89,1967) to(i,k),dz,pwo(i,k)
!1967      format(4e13.5)
#endif
!
...
#ifdef SAS_DET_AUG2018
           drag(i,k) = max(xlamb(i,k),xlamud(i,k))
#else
...
#else
           qlko_ktcon(i) = dq * sigma(i)
           qcko(i,k) = qrch + dq * (1.-sigma(i))
#endif
#else
           qlko_ktcon(i) = dq
           qcko(i,k) = qrch
...
!
   ...
   do i = its,ite
     if(cnvflg(i)) then
       kt1 = ktcon(i)
       kb1 = kbcon(i)
       wpdtop = sqrt(uo(i,kt1)**2 + vo(i,kt1)**2)
       wpdbot = sqrt(uo(i,kb1)**2 + vo(i,kb1)**2)
       vshear(i) = sqrt((uo(i,kt1)-uo(i,kb1)) ** 2 + (vo(i,kt1)-vo(i,kb1)) ** 2)
!
       if(wpdtop.gt.wpdbot) then
         shsign(i) =  1.
       else
         shsign(i) = -1.
       endif
...
!
   do i = its,ite
     if(cnvflg(i)) then
       ...
       kt1 = ktcon(i)
       kb1 = kbcon(i)
       vshear(i) = 1.e3 * shsign(i) * vshear(i) / (zi(i,kt1)-zi(i,kb1))
#else
...
!
#ifdef SAS_CCN
       ccn_f = max(0.08*log(xccn(i,kbcon(i))/1000000.)+0.63,1.)
#else
       ccn_f = 1.0
#endif
       edt_s(i) = edt(i)
   ...
   do k = kts,kmax1
     do i = its,ite
       if(cnvflg(i)) then
         ...
         dp = .5 * (p(i,k+1) - p(i,k))
#ifdef WRF
         es = 0.01*fsvp(to(i,k+1),1,rd_,rv_,cvap_,cliq_,cice,xlv0,xls,psat,t0c_)
         pprime = p(i,k+1) + (eps-1.) * es
#else
#ifdef ICE
         es = 1.e3 * fpvs(to(i,k+1))
...
#ifdef WRF
         po = .5 * (p(i,k) + p(i,k+1))
         qeso(i,k) = 0.01*fsvp(to(i,k),1,rd_,rv_,cvap_,cliq_,cice,xlv0,xls,psat,t0c_)
         qeso(i,k) = eps * qeso(i,k) / (po + (eps-1.) * qeso(i,k))
         qeso(i,k) = max(qeso(i,k),qmin_)
         qo(i,k)   = max(qo(i,k), 1.0e-10)
#else
...
#ifdef CPS_KSAS_DIUR
!
!  compute mean velocity between kb kbcon
!
   do i = its,ite
#ifdef WRF
     if(cnvflg(i).and.slimsk(i).eq.2.) then
#else
     if(cnvflg(i).and.slimsk(i).eq.0.) then
#endif
       wc(i) = 0.
       sumx(i) = 0.
     endif
...
#ifdef AAA
       write(75,9986) lat,i,kb(i),kbcon(i),hpbl(i),hpbl_hold(i),dt2,           &
                      zl(i,kbcon(i)),wc(i),sumx(i)
       9986 format('APBL',4i5,6e13.5)
#endif
     endif
   enddo
#endif
...
!
     if(cnvflg(i)) then
       write(75,9985) lat,i,aa1(i),apbl(i),dtconv(i),f(i),xk(i),               &
                      xmb(i),xmbmax(i)
       9985 format('Large-FC',2i5,7e13.5)
     endif
#endif
   enddo
!
   ...
   do k = kts,kmax
     do i = its,ite
       if(cnvflg(i)) then
         ...
         vo(i,k) = v1(i,k)
#ifdef WRF
         qeso(i,k) = 0.01*fsvp(t1(i,k),1,rd_,rv_,cvap_,cliq_,cice,xlv0,xls,psat,t0c_)
         qeso(i,k) = eps * qeso(i,k) / (p(i,k) + (eps-1.) * qeso(i,k))
#else
#ifdef ICE
         qeso(i,k) = 1.e3 * fpvs(t1(i,k))
#else
         qeso(i,k) = 1.e3 * fpvs0(t1(i,k))
#endif
         qeso(i,k) = eps * qeso(i,k) / (p(i,k) - qeso(i,k))            ! MR
#endif
         qeso(i,k) = max(qeso(i,k),qmin_)
   ...
   do k = kts,kmax
     do i = its,ite
       if(cnvflg(i).and.k.le.ktcon(i)) then
         ...
         dp = 1000. * del(i,k)
#else
         u1(i,k) = u1(i,k) + dellau(i,k) * xmb(i) * dt2 / rcs
         v1(i,k) = v1(i,k) + dellav(i,k) * xmb(i) * dt2 / rcs
         dp = del(i,k)
#endif
   ...
   if(iope) then
     ...
     do i = its,ite
       if(i.eq.(its+ite)/2) then
         print*,'before evap ',delhbar(i),delqbar(i),deltbar(i)
         print*,'del_print1 ', delubar(i),delvbar(i)
       endif
     enddo
#ifndef WRF
   endif
...
#ifdef SAS_CCN
         ccn_f = max(0.08*log(xccn(i,k)/1000000.)+0.63,1.)
#else
         ccn_f = 1.0
#endif
#ifdef SAS_CCN_OCT2017
...
!
   ...
   if(iope) then
     ...
     do i = its,ite
       if(i.eq.(its+ite)/2) then
         print*,'after evap ',delhbar(i),delqbar(i),deltbar(i)
         print*,'after evap ',delhbar(i),delqbar(i)*hvap_,deltbar(i)*cp_
...
#else
!             tem1 = mpp0(t1(i,k))
             if(t1(i,k).le.(2.7316e+2 - 23.))then
               tem1 = 1.
             else if ( t1(i,k).lt.2.7316e+2 ) then
               tem1 = 1. - ( (t1(i,k)-(2.7316e+2 - 23.))/((2.7316e+2)-(2.7316e+2 - 23.)) )**2.
             else
               tem1 = 0.
             endif
             if(ncloud.ge.4) then
...
!-------------------------------------------------------------------------------
   REAL FUNCTION fsvp(t,ice,rd,rv,cvap,cliq,cice,hvap,hsub,psat,t0c)
!-------------------------------------------------------------------------------
   IMPLICIT NONE
!-------------------------------------------------------------------------------
...
!
   ...
   if(t.lt.ttp.and.ice.eq.1) then
     fsvp=psat*(tr**xai)*exp(xbi*(1.-tr))
   else
     fsvp=psat*(tr**xa)*exp(xb*(1.-tr))
   endif
!
   if (t.lt.180.) then
     tr=ttp/180.
