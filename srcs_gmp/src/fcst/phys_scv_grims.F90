...
!
#undef DEBUG
#define GRIMSCV_STRONG
!
#ifndef WRF
#include <define.h>
#else
#define FEB2013_GRIMSCV
#endif
!
#ifdef WRF
#undef GRIMSCV_STRONG
#endif
!
#ifdef WRF
...
#else
   subroutine phys_scv_grims(q,t,prsi,prsik,delprsi,prsl,prslk,zl,             &
#endif
                       wstar,hpbl,delta,                                       &
                       dt,cp,g,xlv,rd,rv,                                      &
                       icps,kpbl,levshc,                                       &
...
#ifndef WRF
! input argument
!-- q           2d specific humidity (kgkg-1)
!-- t           2d dry hydrostatic temperature (k)
!-- prsi        2d pressure (pa) at interface level
!-- prsik       2d exner function at interface level (dimensionless)
!-- delprsi     2d pressuer depth (pa) between interface levels
!-- prsl        2d pressure (pa)
!-- prslk       2d exner function (dimensionless)
!-- zl          2d z at full level (m)
...
!
! output argument
!-- q           2d adjusted specific humidity (kgkg-1)
!-- t           2d adjusted dry hydrostatic temperature (k)
!
! subprograms called:
!   phys_moist_adiabat_pa  - computes moist adiabat and returns cloud values
!   scv_tri_diagonal_grims - solves tridiagonal matrix problem
!
!  ::: structure :::
!
!    [phys_scv_grims] --- [scv_tri_diagonal_grims]
! 
!-------------------------------------------------------------------------------
#endif
!
...
!
   ...
   real                 ::  qeso2(its:ite*kte),rh2(its:ite)
   real                 ::  depth(its:ite),zdiff1(its:ite),zdiff2(its:ite)
   real                 ::  hmin(its:ite),hmax(its:ite)
   real                 ::  z(its:ite,kts:kte)
   real                 ::  heo(its:ite,kts:kte)
   real                 ::  heso(its:ite,kts:kte)
#endif
...
#ifdef DEBUG
         if(index2(i).eq.149)                                                  &
           print*,' heo heso z t2 q2 ',heo(i,k),heso(i,k),z(i,k),t2(ik),q2(ik)
#endif
       endif
     enddo
   enddo
...
#ifdef DEBUG
       if(index2(i).eq.149)                                                    &
         print*,'kpbl kb kbcon zdif1 zdif2 ',kpbl(index2(i)),kb(i),kbcon(i),   &
           zdiff1(i),zdiff2(i)
#endif
     endif
   enddo
#endif /* end of FEB2013_GRIMSCV */
!
...
!
   do i = 1,n2
     if(lshc(index2(i))) then
#ifdef DEBUG
       if(index2(i).eq.149) print*,'ktop lmin hmin ',ktop(i),lmin(i),hmin(i)
#endif
...
#endif
#else
         xkzh = min(max(xkzfac*(1.-height/depth(i))**2,.1),5.)
#endif
#ifdef DEBUG
         if(index2(i).eq.149) print *,'lon k kb kt d h kz rh ws kpbl hpbl '    &
          ,index2(i)*1.875,k,kbot(i),ktop(i),                                  &
          depth(i),height,xkzh,rh2(i),wsfac*wstar(index2(i)),kpbl(index2(i)),  &
          hpbl(index2(i))
#endif
       else
         xkzh = 0.
