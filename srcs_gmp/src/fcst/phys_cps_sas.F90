...
#else
     do i = its,ite
       pratec(i,j)=rain(i)*1000./(stepcu*dt)
       raincv(i,j)=rain(i)*1000./(stepcu)
#endif
       hbot(i,j) = kbot(i)
       htop(i,j) = ktop(i)
     enddo
...
!-------------------------------------------------------------------------------
   subroutine nsas2d(delt,delx,del,prsl,prsi,prslk,zl,zi,                      &
            ...
            its,ite, jts,jte, kts,kte)
#else    
   subroutine phys_cps_sas(delt,delx,del,prsl,prsi,prslk,prsik,zl,zi,          &
#ifdef DET_HYDRO
            q2,                                                                &
#endif
#ifdef SAS_CCN
...
!            - in case of the WRF --> qc2(cloud), qi2(ice)
...
!-------------------------------------------------------------------------------
#ifndef WRF
   use constant,only : cp_,cliq_,cvap_,g_,hvap_,pi_,qmin_,rd_,rv_,t0c_,fv_,cv_
#ifdef DFS
   use dfsvar, only : iope
#else
   use comio, only : iope
#endif
   use comconsts, only : kbmaxin,kbmin,kmaxin
#endif
#ifdef CHEM
   use dao_mod, only : c_dtrain=>dtrain
#endif
!-------------------------------------------------------------------------------
   implicit none
!-------------------------------------------------------------------------------
...
#else
   real,parameter  ::  evfacts = 0.07, evfactl = 0.07
#endif
!
#ifdef CLM_CWF
   real,parameter  ::  cgsref = 4.0e-8, acc0 = 3.0
#endif
#ifdef DET_HYDRO
...
!
   ...
   integer         ::  lat,                                                    &
#else
   integer         ::  lat,jcap,dx_factor_nsas,                                &
#endif
                       ncloud,                                                 &
                       ids,ide, jds,jde, kds,kde,                              &
                       ims,ime, jms,jme, kms,kme,                              &
...
#else
   real            ::  del(ims:ime,kms:kme),                                   &
                       prsl(ims:ime,kms:kme),prslk(ims:ime,kms:kme),           &
                       prsi(ims:ime,kms:kme+1),prsik(ims:ime,kms:kme+1),       &
                       zl(ims:ime,kms:kme),zi(ims:ime,kms:kme+1),              & 
                       q1(ims:ime,kms:kme),t1(ims:ime,kms:kme),                &
                       u1(ims:ime,kms:kme),v1(ims:ime,kms:kme),                &
#endif
#ifdef EXPLICIT_CLOUDINESS
                       qci(ims:ime,kms:kme),qrs(ims:ime,kms:kme),              &
#endif
#ifdef WRF
                       dot(its:ite,kts:kte)
#else
...
#else
   real            ::  q2(ims:ime,kms:kme,ncloud)
#endif
#endif
#ifdef SAS_CCN 
   real            ::  xccn(its:ite,kts:kte)
#endif        
!
#ifdef WRF
   real            ::  rain(its:ite)
#else
   real            ::  cldwrk(ims:ime),rain(ims:ime)
   real            ::  hpbl(ims:ime),gamt(ims:ime),gamq(ims:ime)
...
!
#ifdef CLM_CWF
   real            ::  cgs2(ims:ime,kts:kte)
#endif
#ifdef MUL_CLDTOP
   integer         ::  kt2(ims:ime)
   real            ::  xkt2(ime*ncldtop)
#endif
#ifdef SAS_DIAG
   real            ::  dcu(ims:ime,kms:kme), dcv(ims:ime,kms:kme),             &
                       dct(ims:ime,kms:kme), dcq(ims:ime,kms:kme),             &
                       dch(ims:ime,kms:kme
   real            ::  fcu(ims:ime,kms:kme), fcd(ims:ime,kms:kme)
   real            ::  cbmf(ims:ime), deltb(ims:ime),                          &
                       delqb(ims:ime), delhb(ims:ime)
#else
#ifdef GWDC
   real            ::  dct(ims:ime,kms:kme)
   real            ::  fcu(ims:ime,kms:kme), fcd(ims:ime,kms:kme)
#endif
#endif
!
...
#else
   real            ::  xlamb(its:ite),xlamd(its:ite)
#endif
   real            ::  excess(its:ite)
   real            ::  plcl(its:ite)
...
#ifdef DET_HYDRO
#ifndef WRF
   real            ::  ql(its:ite,kts:kte,ncloud)
#endif
   real            ::  qlko_ktcon(its:ite)
#endif
#ifdef MUL_CLDTOP
   real            ::  sumz(its:ite,kts:kte),sumh(its:ite,kts:kte)
   real            ::  t1mct(its:ite,kts:kte),q1mct(its:ite,kts:kte)
   real            ::  ncldsum(its:ite)
#endif
!
#ifndef WRF
   real            ::  fpvs,fpvs0
...
#ifndef NCEP2010
   logical         ::  dwnflg(its:ite), dwnflg2(its:ite)
#endif 
   real            ::  dx_factor
!
   ...
   if (ncloud.ge.2) then
     do k = kts,kte
       do i = its,ite
         ql(i,k,1) = q2(i,k,1)
         if (ncloud.ge.4) then
           ql(i,k,1) = q2(i,k,3)   !  ql (1) = ice, ql(2) = liquid
           ql(i,k,2) = q2(i,k,1)   !  q2 (1) = qc, q2 (3) = qi
         endif
       enddo
     enddo
   endif
...
!
   do i = its,ite
     if(cnvflg(i)) then
       ...
       if(zl(i,indx).le.hpbl(i)) then
         theb = thec
       else
         delza = max(zl(i,indx) - hpbl(i),qmin_)
         thekb = tvo(i,indx)/prslk(i,indx)*prsik(i,1)
         thekh = tvo(i,indp)/prslk(i,indp)*prsik(i,1)
...
!
   do i = its,ite
     if(cnvflg(i)) then
       indx = kb(i)
       rh = min(max(qo(i,indx)/qeso(i,indx),qmin_),1.)
...
!
   do i = its,ite
     if(cnvflg(i)) then
       ...
       if((hkbo(i)+excess(i)*cp_).lt.heso(i,klcl(i))) cnvflg(i) = .false.
     endif
   enddo
!
! upto here, kbcon is lcl. put the cloud bottom as lfc
!
   do i = its,ite
     ...
     do i = its,ite
       if((.not.lclflg).and.cnvflg(i)) then
         if(flg(i).and.k.gt.kb(i).and.k.ge.klcl(i)) then
           hsbar(i) = heso(i,k)
           if((hkbo(i)+excess(i)*cp_).gt.hsbar(i)) then
             flg(i)   = .false.
             kbcon(i) = k
           endif
         endif
...
!
   do i = its,ite
     ...
     if(cnvflg(i)) then
       if(kb(i).eq.1) then
         dz = .5 * (zl(i,kbcon(i)) + zl(i,kbcon(i)-1)) - zl(i,1)
       else
         dz = .5 * (zl(i,kbcon(i)) + zl(i,kbcon(i)-1))                         &
            - .5 * (zl(i,kb(i)) + zl(i,kb(i)-1))
       endif
...
!
   do k = kmax,kts,-1
     do i = its,ite
       if(dbyo(i,k).ge.0..and.flg(i).and.k.gt.kbcon(i).and.k.le.ktcon(i)) then
         ktdown(i) = k
         flg(i) = .false.
       endif
     enddo
...
! Feb 10 2004
!     after this line...
!     consideration of multiple clouds not interactive from each other
!     -- simple mean of modified T & Q (or U,V) fields due to clouds ...
! yhbyun
!-------------------------------------------------------------------
!
   ...
   do k = kts1,kmax1
     do i = its,ite
       if(cnvflg(i).and.k.gt.jmin(i).and.k.le.ktcon(i)) then
         sumz(i,k) = sumz(i,k-1) + .5 * (zl(i,k+1) - zl(i,k-1))
         sumh(i,k) = sumh(i,k-1) + .5 * (zl(i,k+1) - zl(i,k-1))* heo(i,k)
       endif
     enddo
   enddo
!
...
!
   ncldt = 1
!
2004 if (ncldt.gt.ncldtop) goto 2005
!
   do i = its,ite
     if(cnvflg(i)) then
       incld = (ncldt-1)*ite + i
       xkt2cld = xkt2(incld)
       kt2(i) = nint(xkt2cld*float(ktcon(i)-jmin(i))-.5)+jmin(i)+1
       tem1 = (hcko(i,jmin(i)) - heso(i,kt2(i)))
       tem2 = (sumz(i,kt2(i)) * heso(i,kt2(i)) - sumh(i,kt2(i)))
       if (abs(tem2) .gt. 0.000001) THEN
         xlamb(i) = tem1 / tem2
         xlamb(i) = max(xlamb(i),0.)
   ...
   do i = its,ite
     if(cnvflg(i)) then
       dz = .5 * (zl(i,ktcon(i))+zl(i,ktcon(i)-1))                             &
          - .5 * (zl(i,kbcon(i))+zl(i,kbcon(i)-1))
       xlamb(i) = -1. / dz
     endif
   enddo
!
#endif
#endif /* MUL_CLDTOP end */
#ifndef NCEP2010
   do i = its,ite
     dwnflg(i) = cnvflg(i)
     dwnflg2(i) = cnvflg(i)
...
!
   ...
   do k = kts1,kmax1
     do i = its,ite
       if(cnvflg(i).and.k.gt.jmin(i).and.k.le.kt2(i)) then
...
#ifdef EXPLICIT_CLOUDINESS
!             qci(i,k) = qlk
!              if (i.eq.1) write(89,1967) to(i,k),dz,c0*dz,pwo(i,k)
!1967          format(4e13.5)
#endif
#endif /* DBG end */
#else /* NEW_AUTO */
...
!
#ifdef SAS_CCN
           alpha1 = min((-0.7*log(xccn(i,k))+24.)*0.0001,0.002) 
           beta1 = 0.07
#else
           alpha1 = min((-0.7*log(100.)+24.)*0.0001,0.002)  
           beta1 = 0.07
#endif
!
           etah = .5 * (eta(i,k) + eta(i,k-1))
...
#ifdef EXPLICIT_CLOUDINESS
!             qci(i,k) = qlk
!              if (i.eq.1) write(89,1967) to(i,k),dz,c0dz,pwo(i,k)
!1967          format(4e13.5)
#endif
#endif
#endif /* ~NEW_AUTO end */
   ...
   do k = kts,kmax
     do i = its,ite
       if(cnvflg(i).and.k.gt.kbcon(i).and.k.lt.ktcon(i)) then
         dz1 = zl(i,k) - zl(i,k-1)
         gamma = el2orc * qeso(i,k-1) / (to(i,k-1)**2)
         rfact =  1. + fv_ * cp_ * gamma* to(i,k-1) / hvap_
         aa1(i) = aa1(i) +dz1 * (g_ / (cp_ * to(i,k-1)))                       &
                  * dbyo(i,k-1) / (1. + gamma)* rfact
         aa1(i) = aa1(i)+dz1 * g_ * fv_ *                                      &
...
!
#ifdef SAS_CCN
       ccn_f = max(0.08*log(xccn(i,kbcon(i)))+0.63,1.)
       edt_s(i) = edt(i)
   ...
   do i = its,ite
     ...
     if(cnvflg(i)) then
       ...
       if(kbdtr(i).gt.1) then
         dz = .5 * zl(i,kbdtr(i)) + .5 * zl(i,kbdtr(i)-1)                      &
            - zl(i,1)
         xlamd(i) = log(beta) / dz
       endif
#endif
...
#else
          dbyo(i,k) = hcdo(i,jmin(i)) - heso(i,k)
#ifdef UV_CMT
         dz = zi(i,k+2) - zi(i,k+1)
         if(k.ge.kbcon(i)) then
           tem  = xlamde * dz
   ...
   do k = kmax1,kts,-1
     do i = its,ite
       if(cnvflg(i).and.k.lt.jmin(i)) then
         ...
         qcdo(i,k) = qrcdo(i,k)
#else
         pwdo(i,k) = etad(i,k+1) * qcdo(i,jmin(i)) -etad(i,k) * qrcdo(i,k)
         pwdo(i,k) = pwdo(i,k) - detad *.5 * (qrcdo(i,k) + qrcdo(i,k+1))
         qcdo(i,jmin(i)) = qrcdo(i,k)
#endif
   ...
   do i = its,ite
     if(cnvflg(i)) then
       ...
       dellaq(i,1) = edto(i) * etad(i,1) * (qcdo(i,1)                          &
                   - qo(i,1)) * g_ / dp
#else
       dellah(i,1) = edto(i) * etad(i,1) * (hcdo(i,jmin(i))                    &
                   - heo(i,1)) * g_ / dp
       dellaq(i,1) = edto(i) * etad(i,1) * (qcdo(i,jmin(i))                    &
                   - qo(i,1)) * g_ / dp
#endif
#ifdef UV_CMT
...
#else
         - aup * deta * dv2                                                    &
         + adw * edto(i) * detad * hcdo(i,jmin(i))) * g_ / dp
#endif
...
#else
         - aup * deta * dv2q                                                   &
         +adw*edto(i)*detad*.5*(qrcdo(i,k)+qrcdo(i,k-1))) * g_ / dp
#endif
#ifdef UV_CMT
     ...
     do i = its,ite
       if(cnvflg(i).and.k.gt.kb(i).and.k.lt.ktcon(i)) then
         ...
         if(dq.gt.0.) then
           etah = .5 * (eta(i,k) + eta(i,k-1))
           qlk = dq / (eta(i,k) + etah * c0 * dz)
           xaa0(i) = xaa0(i) - (zl(i,k) - zl(i,k-1)) * g_ * qlk
#endif
           qcko(i,k) = qlk + xqrch
           xpw = etah * c0 * dz * qlk
           xpwav(i) = xpwav(i) + xpw
...
#else
       if(cnvflg(i).and.k.gt.kbcon(i).and.k.lt.ktcon1(i)) then
         dz1 = zl(i,k) - zl(i,k-1)
         gamma = el2orc * qeso(i,k-1) / (to(i,k-1)**2)
         rfact =  1. + fv_ * cp_ * gamma                                       &
                  * to(i,k-1) / hvap_
...
!
#ifdef AAA
       write(75,9976) lat,i,kbcon(i),pdot(i),acrtfct(i),dtconv(i)
       9976 format('     SAS',3i5,3e13.5)
#endif
     endif
#else
!
     if(cnvflg(i)) then
       ...
       sump = 0.0
       p1 = p(i,1)*0.01
       do k = kts, ktcon(i)             ! sfc to cld_top
         p2 = p(i,k+1)*0.01
         delp = p1-p2
         sumc = sumc + delp * 0.5 * (cgs2(i,k)+cgs2(i,k+1))
         sump = sump + delp
         p1 = p2
       enddo
       vintcgs = sumc/sump
       acc = vintcgs/cgsref           ! vertically integrated 
       acrtfct(i) = (pi_/2.0 - atan(acc-acc0))/pi_
!
! time tuning
!
       dtconv(i) = 1800.
!
#ifdef AAA
       write(75,9986) lat,i,kbcon(i),
  +                  cgs2(i,kbcon(i)),vintcgs,acrtfct(i),dtconv(i)
9986   format('     SAS',3i5,4e13.5)
!
#endif
     endif
#endif
...
!
     if(cnvflg(i)) then
#ifdef AAA
       write(75,9985) lat,i,aa1(i),acrt(i),acrtfct(i),f(i),xk(i),              &
                      xmb(i)
       9985 format('Large-FC',2i5,6e13.5)
#endif
     endif
!
   enddo
...
!
   do i = its,ite
     if(cnvflg(i)) then
#ifdef AAA
!       write(75,9982) lat,i,delhbar(i),deltbar(i),delqbar(i),                  &
       write(75,9982) lat,i,delhbar(i)-cp_*deltbar(i)-hvap_*delqbar(i),         &
                      xmb(i),dt2
       9982 format('DELxBAR ',2i5,5e13.5)
#endif
     endif
   enddo
!
...
!           write(75,9983) lat,i,k,pwo(i,k),edto(i),pwdo(i,k),aup,adw
!           9983 format('Qcps',3i5,3e13.5,2x,2f5.1)
#endif
       endif
       if(cnvflg(i).and.flg(i).and.k.lt.ktcon(i)) then
#ifdef SAS_CCN
         ccn_f = max(0.08*log(xccn(i,k))+0.63,1.)
#endif
!
#ifdef NCEP2010
#ifdef SAS_CCN
...
#ifdef SAS_CCN
         evef = evfacts * ccn_f
         if(slimsk(i).eq.1.) evef = evfactl * ccn_f
#else
         evef = evfacts
         if(slimsk(i).eq.1.) evef = evfactl
#endif
#endif
         ...
         if(rain(i).gt.0..and.qevap(i).gt.0.) then
           ...
           delq(i) =  + qevap(i)/dt2
#ifdef AAA
           write(75,9983) lat,i,k,qevap(i),(hvap_/cp_)*qevap(i),q1(i,k),t1(i,k)
           9983 format('EVAP',3i5,4e13.5)
#endif
         endif
...
#ifdef EXPLICIT_CLOUDINESS
       if(cnvflg(i).and.k.lt.ktcon(i)) then
         qrs(i,k) = max(qrsk(i,k) + min(qcond(i),0.),0.)
         qci(i,k) = max(qcirs(i,k) - qrs(i,k),0.)
       endif
#endif
     enddo
   enddo
!
   ...
   do i = its,ite
     if(cnvflg(i)) then
       write(75,9981) lat,i,delhbar(i),deltbar(i),delqbar(i)
       9981 format('aft_evap',2i5,3e13.5)
     endif
   enddo
#endif
#ifdef DBG
...
#endif
   do i = its,ite
     if (i .eq. (its+ite)/2) then
       print*,'del_print1 ',delhbar(i),delqbar(i),deltbar(i)
#ifdef UV_CMT
       print*, 'del_print2',delubar(i),delvbar(i)
#endif
     endif
   enddo
#ifndef WRF
...
! 
   do i = its,ite
     if (cnvflg(i)) then
       ncldsum(i) = ncldsum(i) + 1
       do k = kts,kmax
         t1mct(i,k) = t1mct(i,k) + t1(i,k)    
         q1mct(i,k) = q1mct(i,k) + q1(i,k)    
       enddo
     endif
   enddo
!
...
2005  do k = kts,kmax
     do i = its,ite
       if (ncldsum(i).gt.1) then
         t1(i,k) = t1mct(i,k)/real(ncldsum(i))
         q1(i,k) = q1mct(i,k)/real(ncldsum(i))
       endif
     enddo
   enddo
#endif
...
!
   do k = kts,kmax
     do i = its,ite
       if (cnvflg(i).and.rain(i).gt.0.) then
         dch(i,k) = dellah(i,k) * xmb(i)
         dcq(i,k) = dellaq(i,k) * xmb(i)
         dct(i,k) = (dellah(i,k) -hvap_*dellaq(i,k)) *xmb(i) /cp_
       endif
     enddo
   enddo
!
   ...
   enddo
!
! cloud mass flux calculation
!
   do k = kmax,kts,-1
     do i = its,ite
       if(cnvflg(i).and.k.le.ktcon(i).and.rain(i).gt.0.) then
         aup = 1.
         if(k.le.kb(i)) aup = 0.
         adw = -1.
         if(k.gt.jmin(i)) adw = 0.
         fcu(i,k) = aup * xmb(i) * eta(i,k)
         fcd(i,k) = adw * edto(i) * xmb(i) * etad(i,k)
       endif
     enddo
   enddo
#endif
#ifdef GWDC
!
! save net mass flux : fcu, fcd in gwdc option
!
   do k = kts,kmax
     do i = its,ite
       if (cnvflg(i).and.rain(i).gt.0.) then
...
#ifdef CHEM
!
! save detrainment rate for chemistry convection
!
   do k = kts1,kmax1
     do i = its,ite
       if(cnvflg(i).and.k.le.ktcon(i).and.rain(i).gt.0.) then
         ...
         c_dtrain(i,lat,k) = ( aup * tem1 * eta(i,k-1) +                       &
                             adw * edto(i) * ptem1 * etad(i,k) ) * dz * xmb(i)
       endif
     enddo
   enddo
#endif
#endif  /* end define NSAS */
#ifdef WRF
#ifdef NSAS
...
#ifdef NSAS
   return
   end subroutine phys_cps_sas
#endif
#endif /* WRF end */
!
#ifdef WRF  /* start define WRF */
!-------------------------------------------------------------------------------
...
#else /* ~WRF */
   subroutine phys_scv_hanpan(jcap,delt,del,prsl,prsi,prslk,prsik,zl,          &
                 phil,ncloud,                                                  &
#ifdef DET_HYDRO
                 q2,                                                           &
#endif
                 q1,t1,rain,kbot,ktop,icps,slimsk,dot,hpbl,                    &
                 u1,v1,rcs,                                                    &
                 hfx,qfx,                                                      &
                 ids,ide, jds,jde, kds,kde,                                    &
                 ims,ime, jms,jme, kms,kme,                                    &
...
#else
! subprogram:    phys_scv_hanpan  computes shallow-convective heating and moisng
#endif
!
...
!
   ...
   real            ::  del(ims:ime,kms:kme),                                   &
                       prsl(ims:ime,kms:kme),prslk(ims:ime,kms:kme),           &
                       prsi(ims:ime,kms:kme+1),prsik(ims:ime,kms:kme+1),       &
                       zl(ims:ime,kms:kme),phil(ims:ime,kms:kme)
#endif
   integer         ::  ncloud
   real            ::  slimsk(ims:ime)
#ifdef WRF
...
#else
#ifdef DET_HYDRO
   real            ::  ql(ims:ime,kms:kme,2),q2(ims:ime,kms:kme,ncloud)
#endif
   real            ::  q1(ims:ime,kms:kme),                                    &
                       t1(ims:ime,kms:kme),                                    &
                       u1(ims:ime,kms:kme),                                    &
                       v1(ims:ime,kms:kme)
...
#else
   real            ::  rain(ims:ime)
   integer         ::  kbot(ims:ime),ktop(ims:ime)
#endif
!
!  local variables and arrays
!
...
#else
   integer         ::  kb(ims:ime), kbcon(ims:ime), kbcon1(ims:ime),           &
                       ktcon(ims:ime), ktcon1(ims:ime),                        &
                       kbm(ims:ime), kmax(ims:ime)
#endif
!
#ifdef WRF
   real            ::  aa1(its:ite),                                           &
...
#else
   real            ::  aa1(ims:ime),                                           &
                       delhbar(ims:ime), delq(ims:ime),   delq2(ims:ime),      &
                       delqbar(ims:ime), delqev(ims:ime), deltbar(ims:ime),    &
                       deltv(ims:ime),   edt(ims:ime),                         &
                       wstar(ims:ime),   sflx(ims:ime),                        &
                       pdot(ims:ime),    po(ims:ime,kms:kme),                  &
                       qcond(ims:ime),   qevap(ims:ime),  hmax(ims:ime),       &
                       rntot(ims:ime),   vshear(ims:ime),                      &
                       xlamud(ims:ime),  xmb(ims:ime),    xmbmax(ims:ime)
#ifdef UV_CMT
   real            ::  delubar(ims:ime), delvbar(ims:ime)
#endif
#endif
!
   real            ::  cincr
...
#else
   real            ::  p(ims:ime,kms:kme),       to(ims:ime,kms:kme),          &
                       qo(ims:ime,kms:kme),      qeso(ims:ime,kms:kme),        &
                       uo(ims:ime,kms:kme),      vo(ims:ime,kms:kme)
#endif
!
!  cloud water
!
   ...
   real            ::  qlko_ktcon(its:ite),     dellal(its:ite,kts:kte),       &
                       ...
                       pwo(its:ite,kts:kte)
#else
   real            ::  qlko_ktcon(ims:ime),     dellal(ims:ime,kms:kme),       &
                       dbyo(ims:ime,kms:kme),   zo(ims:ime,kms:kme),           &
                       xlamue(ims:ime,kms:kme),                                &
                       heo(ims:ime,kms:kme),    heso(ims:ime,kms:kme),         &
                       dellah(ims:ime,kms:kme), dellaq(ims:ime,kms:kme),       &
#ifdef UV_CMT
                       dellau(ims:ime,kms:kme), dellav(ims:ime,kms:kme),       &
                       ucko(ims:ime,kms:kme),   vcko(ims:ime,kms:kme),         &
#endif
                       hcko(ims:ime,kms:kme),   qcko(ims:ime,kms:kme),         &
                       eta(ims:ime,kms:kme),    zi(ims:ime,kms:kme),           &
                       pwo(ims:ime,kms:kme)
#endif
!
#ifdef WRF
...
#else
   logical         ::  totflg, cnvflg(ims:ime), flg(ims:ime)
   real            ::  fpvs,fpvs0
#endif
#ifdef DBG
!
...
!
   real            ::  ud_mf(its:ite,kts:kte),dt_mf(its:ite,kts:kte)
#endif
!
!  physical parameters
!
...
#else
   real,parameter  ::  cpoel=cp_/hvap_,el2orc=hvap_*hvap_/(rv_*cp_)
   real,parameter  ::  terr=0.,c0=.002,c1=5.e-4
   real,parameter  ::  fact1=(cvap_-cliq_)/rv_,fact2=hvap_/rv_-fact1*t0c_
   real,parameter  ::  cincrmax=180.,cincrmin=120.,dthk=25.
   real,parameter  ::  h1=0.33333333
     ...
     do i=its,ite
       if (cnvflg(i) .and. k .le. kmax(i)-1) then
         ...
         heso(i,k) = .5 * g_ * (zl(i,k) + zl(i,k+1)) +                         &
                        cp_ * to(i,k) + hvap_ * qeso(i,k)
#else
         heo(i,k)  = .5 * g_ * (zo(i,k) + zo(i,k+1)) +                         &
                        cp_ * to(i,k) + hvap_ * qo(i,k)
         heso(i,k) = .5 * g_ * (zo(i,k) + zo(i,k+1)) +                         &
                        cp_ * to(i,k) + hvap_ * qeso(i,k)
#endif
         uo(i,k)   = .5 * (uo(i,k) + uo(i,k+1))
         vo(i,k)   = .5 * (vo(i,k) + vo(i,k+1))
...
#else
      return
      end subroutine phys_scv_hanpan
#endif
#endif   /* end define NSCV */
#ifdef WRF
!
END MODULE module_cu_nsas
#endif
!
#if defined(CLM_CWF) && !defined(WRF)
!------------------------------------------------------------------------------
   subroutine sas_bh2007_avg(cgs,colrad)
!------------------------------------------------------------------------------
   use constant, only  : pi_,rerth_
   use paramodel, only : lonf2_,latg2_,lonf_,latg_,levs_
#ifdef MP
   use paramodel, only : lonf2p_,latg2p_,lonfp_,latgp_,levsp_,levsr_
#endif
#ifdef DFS
   use dfsvar, only    : kler,levsp,jbw,jb,ib,jbwa,jba,iba,iope,levs,latdef
#ifdef MP
   use commpi          ! mype,lerlen,levlen,latlen,latstr
#endif
#else
#ifdef MP
   use commpi
...
#define LATGL latgp_
#define LONFL lonfp_
#define LATG2L latg2p_
#define LEVCL levsp_
#define LEVRL levsr_
#else
#define LATGL latg_
#define LONFL lonf_
#define LATG2L latg2_
#define LEVCL levs_
#define LEVRL levs_
#endif
#endif
   real    cgs(LONFL,LATGL,LEVSG)
   real    colrad(LATG2G)
!
! local
!
   real, parameter                        ::  er=750000.
   real, allocatable,save,dimension(:,:)  ::  cgsx
   real, allocatable, save                ::  wgtx(:,:), wgty(:)
   real                                   ::  delx(LATG2L),dely
   integer,save,allocatable               ::  nx(:)
   integer,save                           ::  ny
   real,dimension(LATGG,LONFL)            ::  cgsk2tmp
   real,dimension(LONFG,LATGG,LEVSG)      ::  cgsf
#ifdef MP
   real,dimension(LONFG,LATGL,LEVCL)      ::  cgsk1
   real,dimension(LONFL,LATGG,LEVRL)      ::  cgsk2
#ifdef BIN_DBG
   real*4 cgs4(LONFG,LATGG)
#endif
#else
   integer,parameter                      ::  mype=0
#endif
   integer                                ::  ne,ns,lonl,lats2,nxmx
   integer                                ::  jj,levc,levr
   integer                                ::  i,j,k,n
   real                                   ::  dsum1,dsum2,wsum
   logical                                ::  first
   data first/.true./
! ...............................................................
#ifdef DFS
...
!
   if (first) then
     ...
     do j = 1,LATG2L
       ...
       jj=j
#endif /* MP end */
       delx(j)=2.e0*pi_*rerth_*sin(colrad(jj))/LONFG
       nx(j)=er/delx(j)
     enddo
!
     nxmx=maxval(nx(1:LATG2L))
     allocate(wgtx(-nxmx:nxmx,LATG2L))
     do j = 1,LATG2L
       do i = -nx(j),nx(j)
         wgtx(i,j)=exp(-(delx(j)*abs(i)/er)**2)
       enddo
     enddo
!
     allocate(cgsx(-nxmx+1:LONFG+nxmx,2))
     dely=pi_*rerth_/LATGG
     ny=er/dely
     allocate(wgty(-ny:ny))
     do j = -ny,ny
       wgty(j)=exp(-(dely*abs(j)/er)**2)
     enddo
   endif    ! first
!
#ifdef MP
#ifdef DFS
   call mpgp2fd(cgs,ib,jbw,cgsf,iba,jbwa,levs_)
#else
   call mpgp2f(cgs,lonf2p_,latg2p_,cgsf,lonf2_,latg2_,levs_)
#endif
   if (iope)then
#ifndef DFS
     call dyn_trans2output_grid(cgsf,LEVSG)
#endif
#ifdef BIN_DBG
     do k = 1,LEVSG
       cgs4(:,:)=cgsf(:,:,k)
...
!
! transpose for full x
!
#ifdef DFS
   call mpgf2yk(cgsf,lonf_,latg_,levs_,1,cgsk1,jbw,levsp) 
#else
   call mpgf2yk(cgsf,lonf_,latg_,levs_,1,cgsk1,latgp_,levsp_) 
#endif
#else /* ~MP */
#ifdef BIN_DBG
   call file_write_bin(333,cgs,LONFG,LATGG,LEVSG,1)
#endif
#endif /* MP end */
!
! sum in x
...
#ifndef MP
#define CGSK1 cgs
     call dyn_trans2output_grid(cgs(1,1,k),1)
#else
#define CGSK1 cgsk1
#endif
     do j = 1,lats2
       jj=LATGL-j+1
       cgsx(1:LONFG,1)=CGSK1(1:LONFG,j ,k)
       cgsx(1:LONFG,2)=CGSK1(1:LONFG,jj,k)
       do i = 1,nx(j)
         cgsx(-i+1,1)=CGSK1(LONFG-i+1,j ,k)
         cgsx(-i+1,2)=CGSK1(LONFG-i+1,jj,k)
         cgsx(LONFG+i,1)=CGSK1(i,j ,k)
         cgsx(LONFG+i,2)=CGSK1(i,jj,k)
       enddo
!
       do i = 1,LONFG
         dsum1=0.0
         ...
         ne=i+nx(j)
         do n = ns,ne
           dsum1=dsum1+cgsx(n,1)*wgtx(n-i,j)
           dsum2=dsum2+cgsx(n,2)*wgtx(n-i,j)
           wsum=wsum+wgtx(n-i,j)
         enddo
#ifdef MP
         cgsk1(i,j ,k)=dsum1/wsum
         cgsk1(i,jj,k)=dsum2/wsum
#else
         cgsk2tmp(j ,i)=dsum1/wsum
         cgsk2tmp(jj,i)=dsum2/wsum
#ifdef BIN_DBG
         cgsf(i,j ,k)=dsum1/wsum
         cgsf(i,jj,k)=dsum2/wsum
#endif
#endif /* MP end */
       enddo
     enddo
...
#ifdef MP
#ifdef DFS
   call mpyk2f(cgsk1,lonf_,jbw,levsp,1,cgsf,latg_,levs_)
#else
   call mpyk2f(cgsk1,lonf_,latgp_,levsp_,1,cgsf,latg_,levs_)
#endif
!
#ifdef BIN_DBG
   if (iope)then
...
#endif
!
! transpose for full y
!
#ifdef DFS
   call mpgf2xk(cgsf,lonf_,latg_,levs_,1,cgsk2,ib,kler)
#else
   call mpgf2xk(cgsf,lonf_,latg_,levs_,1,cgsk2,lonfp_,levsr_)
#endif
!
! sum in y
!
   do k = 1,lerlen(mype)
     do j = 1,LATG2G
       jj=LATGG-j+1
       do i = 1,lonl
         cgsk2tmp(j,i)=cgsk2(i,j,k)
         cgsk2tmp(jj,i)=cgsk2(i,jj,k)
       enddo
     enddo
#endif /* MP end */
!
     do i = 1,lonl
       do j = 1,LATGG
         ...
         ne=min(j+ny,LATGG)
         do n = ns,ne
           dsum1=dsum1+cgsk2tmp(n,i)*wgty(n-j)
           wsum=wsum+wgty(n-j)
         enddo
#ifdef MP
         cgsk2(i,j,k)=dsum1/wsum
#else
         cgs(i,j,k)=dsum1/wsum
#endif
       enddo
     enddo
   enddo
...
#ifndef MP
#ifdef BIN_DBG
   call file_write_bin(333,cgsf,LONFG,LATGG,LEVSG,0)
   call file_write_bin(333,cgs,LONFG,LATGG,LEVSG,0)
!
   do k = 1,levs_
#endif
     call dyn_trans2model_grid(cgs(1,1,k),1)
   enddo
#else /* MP */
#ifdef DFS
   call mpxk2f(cgsk2,ib,latg_,kler,1,cgsf,lonf_,levs_)
#else
   call mpxk2f(cgsk2,lonfp_,latg_,levsr_,1,cgsf,lonf_,levs_)
#endif
!
   if (iope) then
     do k = 1,levs_
       ...
       write(333)cgs4
#endif
       call dyn_trans2model_grid(cgsf(1,1,k),1)
     enddo
   endif
#ifdef DFS
   call mpgf2p(cgsf,lonf2_,latg2_,cgs,ib*2,jb,levs_)
#else
   call mpgf2p(cgsf,lonf2_,latg2_,cgs,lonf2p_,latg2p_,levs_)
#endif
#endif /* ~MP end */
!
#ifdef BIN_DBG
