...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE wsm32D(t, q                                                      &
                   ...
                   ,delt,g, cpd, cpv, rd, rv, t0c                              &
#else
   SUBROUTINE phys_mps_wsm3(t1, q1                                             &
                   ,q2, ncloud,w, p,delz                                       &
                   ,deltim,g, cpd, cpv, rd, rv, t0c                            &
#endif
                   ,ep1, ep2, qmin                                             &
                   ,XLS, XLV0, XLF0, den0, denr                                &
                   ,cliq,cice,psat                                             &
...
!-------------------------------------------------------------------------------
   ...
   REAL, DIMENSION( ims:ime , kms:kme ),                                       &
        INTENT(INOUT) ::                                                       &
                                                                           q1
   REAL, DIMENSION( ims:ime , kms:kme, ncloud ),                               &
        INTENT(INOUT) ::                                                       &
                                                                           q2
#endif
   REAL, DIMENSION( ims:ime , kms:kme ),                                       &
...
!
   ...
   REAL, DIMENSION( its:ite , kts:kte ) ::                                     &
                            q1, t1, pgens, pauts, pacrss, pisds, press, pcons
#endif
!
!=================================================================
...
#ifdef DEBUG
         if(lat.eq.latd.and.i.eq.lond) write(iun,605) i,k,rslope(i,k),         &
                                                      work1(i,k),t(i,k)-t0c
605  format(1x,' ncloud3 lat lev  : rslope vt tc ',2i6,3f20.10)
#endif
       enddo
     enddo
!
...
#else
!
!  forward semi-laglangian scheme (JH), PCM (piecewise constant), PLM (linear)
!
     do k = kte,kts,-1
       do i = its,ite
...
!
#ifdef PCM
     call nislfv_rain_pcm(idim,kdim,den_tmp,denfac,t,delz_tmp,work1,denqrs,    &
                          delqrs,dtcld,1,1)
!
#endif
#ifdef PLM
...
!
#ifdef PCM
     call nislfv_rain_pcm(idim,kdim,den_tmp,denfac,t,delz_tmp,work1c,denqci,   &
                           delqi,dtcld,1,0)
!
#endif
#ifdef PLM
...
#else
         rainncv(i) = fallsum*delz(i,1)/denr*dtcld
...
#else
           snowncv(i) = fallsum_qsi*delz(i,kts)/denr*dtcld
           snow(i) = fallsum_qsi*delz(i,kts)/denr*dtcld + snow(i)
#endif
         endif
       ENDIF
       sr(i) = 0.
...
#ifdef DEBUG
!
     if(lat.eq.latd) write(iun,609) latd,lond,loop,rain(lond)
 609  format(1x,' ncloud3 lat lon loop : rain(mm) ',3i6,f20.2)
#endif
!
!----------------------------------------------------------------
...
!
     do k = kts,kte
       do i = its,ite
         ...
         q2(i,k,1) = qci(i,k)
         q2(i,k,2) = qrs(i,k)
         if(q2(i,k,1).le.qmin)   q2(i,k,1) = 0.0
         if(q2(i,k,2).le.qcrmin) q2(i,k,2) = 0.0
#else
         q2(i,k,1) = q2(i,k,1) +qci(i,k)-max(q2(i,k,1),qmin)
         q2(i,k,2) = q2(i,k,2) +qrs(i,k)-max(q2(i,k,2),qcrmin)
#endif
#endif /* WRF end */
       enddo
     enddo
...
!
!-------------------------------------------------------------------------------
   subroutine slope_wsm3(qrs,den,denfac,t,rslope,rslopeb,rslope2,rslope3,vt,   &
                            its,ite,kts,kte)
!-------------------------------------------------------------------------------
   IMPLICIT NONE
!-------------------------------------------------------------------------------
...
!
!-------------------------------------------------------------------------------
   SUBROUTINE nislfv_rain_pcm(im,km,denl,denfacl,tkl,dzl,wwl,rql,precip,dt,    &
                              id,iter)
!-------------------------------------------------------------------------------
!
...
103  format('  k','        zi','        dz','        ww','        wi',         &
            '        za','       dza','   qq*1000','   qa*1000')
!
     if(iprt.eq.1) then
       do k = 1,km
         write(84,104) k,zi(k),dz(k),ww(k),wi(k),za(k),dza(k),                 &
                       qq(k)*1.e3,qa(k)*1.e3
       enddo
     endif
...
102  format('   k','  kt','  kb',' t-b','    qsum','    zsum', &
            '   qsumt','   zsumt','   qsumb','   zsumb',' qo*1000',' qn*1000')
#endif
!
...
#ifdef DEBUG
         if(iprt.eq.1) then
           write(84,101) k,kt,kb,kt-kb,qsum*1000.,zsum,qsumt*1000.,zsumt,      &
                         qsumb*1000.,zsumb,qq(k)*1000.,qn(k)*1000.
         endif
...
!
!-------------------------------------------------------------------------------
   SUBROUTINE nislfv_rain_plm(im,km,denl,denfacl,tkl,dzl,wwl,rql,precip,dt,    &
                              id,iter)
!-------------------------------------------------------------------------------
!
! for non-iteration semi-Lagrangain forward advection for cloud
...
!
     if( n.le.iter ) then
       ...
       do k = 1,km
!#ifdef DEBUG
!        print*,' slope_wsm3 ',qr(k)*1000.,den(k),denfac(k),tk(k),             &
!                 tmp(k),tmp1(k),tmp2(k),ww(k),wa(k)
!#endif
!
! mean wind is average of departure and new arrival winds
!
...
!
     ...
     qpi(km+1)=qa(km+1)
#ifdef DEBUG
!    call maxmin(km+1,1,qmi,' qmi at arrival interfaces ')
!    call maxmin(km+1,1,qpi,' qpi at arrival interfaces ')
#endif
#ifdef DEBUG
!
     if(iprt.eq.1) then
...
!
     if(iprt.eq.1) then
       do k = 1,km
         write(84,104) k,zi(k),dz(k),ww(k),wi(k),za(k),dza(k),qq(k)*1.e3,      &
                       qa(k)*1.e3
       enddo
     endif
104  format(i3,8f10.3)
...
#ifdef DEBUG
         if(iprt.eq.1) then
           write(84,101) k,kt,kb,kt-kb,qsum*1000.,zsum,                        &
                         qpi(kt)*1000.,qpi(kb)*1000.,                          &
