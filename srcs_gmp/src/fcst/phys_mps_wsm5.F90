...
!
   ...
   ENDIF
   CALL wsm5_host(th(its:ite,kts:kte,jts:jte), pii(its:ite,kts:kte,jts:jte)    &
...
!
   ...
   SUBROUTINE wsm52D(t, q                                                      & 
                    ...
                    ,delt,g, cpd, cpv, rd, rv, t0c                             &
#else
   SUBROUTINE phys_mps_wsm5(t1, q1                                             &
                    ,q2, ncloud, p, delz                                       &
...
#else
       qci(i,k,1) = max(q2(i,k,1),0.0)
       qrs(i,k,1) = max(q2(i,k,2),0.0)
       qci(i,k,2) = max(q2(i,k,3),0.0)
       qrs(i,k,2) = max(q2(i,k,4),0.0)
#endif
     enddo
   enddo
!
...
!          qrs(i,k,2) = max(qrs(i,k,2)-falk(i,k,2)*dtcld/den(i,k),0.)
       ...
       do k = kts,kte
         ...
         enddo
       enddo
       call slope_wsm5(qrs_tmp,den_tmp,denfac,t,rslope,rslopeb,rslope2,        &
                       rslope3,work1,its,ite,kts,kte)
       do k = kte,kts, -1
         do i = its,ite
           work1(i,k,1) = work1(i,k,1)/delz(i,k)
           work1(i,k,2) = work1(i,k,2)/delz(i,k)
...
!
     do k = kts,kte
       do i = its,ite
         ...
         q1(i,k) = q(i,k)
#ifdef NISLQ
         q2(i,k,1) = max(qci(i,k,1),0.)
         q2(i,k,2) = max(qrs(i,k,1),0.)
         q2(i,k,3) = max(qci(i,k,2),0.)
         q2(i,k,4) = max(qrs(i,k,2),0.)
#else
         q2(i,k,1) = q2(i,k,1) +qci(i,k,1)-max(q2(i,k,1),qmin)
         q2(i,k,2) = q2(i,k,2) +qrs(i,k,1)-max(q2(i,k,2),qcrmin)
         q2(i,k,3) = q2(i,k,3) +qci(i,k,2)-max(q2(i,k,3),qmin)
         q2(i,k,4) = q2(i,k,4) +qrs(i,k,2)-max(q2(i,k,4),qcrmin)
#endif
#endif /* WRF end */
       enddo
     enddo
