...
!-------------------------------------------------------------------------------
   SUBROUTINE wdm62D(t, q, qci, qrs, ncr, den, p, delz                         &
                   ,delt,g, cpd, cpv, ccn0, rd, rv, t0c                        &
#else
   SUBROUTINE phys_mps_wdm6(t1, q1, q2, ncloud, n2, p, delz                    &
...
!
   ...
   REAL, DIMENSION( ims:ime , kms:kme, ncloud ),                               &
        INTENT(INOUT) ::                                                       &
                                                                           q2
   REAL, DIMENSION( ims:ime , kms:kme, 3 ),                                    &
        INTENT(INOUT) ::                                                       &
                                                                           n2 
#endif
   REAL, DIMENSION( ims:ime , kms:kme ),                                       &
...
!
     ...
     enddo
!
     call nislfv_rain_plmr(idim,kdim,den_tmp,denfac,t,delz_tmp,work1c,denqci,  &
                           denqci,delqi,dtcld,1,0,0)
!
     do k = kts,kte
       do i = its,ite
...
#else
           snowncv(i) = fallsum_qsi*delz(i,kts)/denr*dtcld + snowncv(i)
...
#else
           graupelncv(i) = fallsum_qg*delz(i,kts)/denr*dtcld                   &
                            + graupelncv(i)
           graupel(i) = fallsum_qg*delz(i,kts)/denr*dtcld + graupel(i)
#endif
         ENDIF
       ENDIF
...
!        (Nc->)
!-------------------------------------------------------------
           naacw(i,k) = (qrs(i,k,2)*nsacw(i,k)+qrs(i,k,3)*ngacw(i,k))          &
                       /(qsum(i,k))
         endif      
!-------------------------------------------------------------
...
!
     do k = kts,kte
       do i = its,ite
         ...
         q2(i,k,5) = q2(i,k,5) +qrs(i,k,3)-max(q2(i,k,5),qcrmin)
         n2(i,k,1) = n2(i,k,1) +ncr(i,k,1)-max(n2(i,k,1),ncmin)
         n2(i,k,2) = n2(i,k,2) +ncr(i,k,2)-max(n2(i,k,2),ncmin)
         n2(i,k,3) = n2(i,k,3) +ncr(i,k,3)-max(n2(i,k,3),nrmin)
#endif
#ifdef NR_LIMIT
          if(qrs(i,k,1).ge.qcrmin) then
...
#else
   SUBROUTINE wdm6init(den0,denr,dens,cl,cpv, allowed_to_read)
#endif
!-------------------------------------------------------------------------------
   IMPLICIT NONE
!-------------------------------------------------------------------------------
...
!
!-------------------------------------------------------------------------------
   SUBROUTINE nislfv_rain_plmr(im,km,denl,denfacl,tkl,dzl,wwl,rql,rnl,precip,  &
                               dt,id,iter,rid)
!-------------------------------------------------------------------------------
!
...
!
!#ifdef DEBUG 
!        print*,' slope_wdm6 ',qr(k)*1000.,den(k),denfac(k),tk(k),tmp(k),      &
!                 tmp1(k),tmp2(k),ww(k),wa(k)
!#endif
!
...
!
!-------------------------------------------------------------------------------
   SUBROUTINE nislfv_rain_plm6(im,km,denl,denfacl,tkl,dzl,wwl,rql,rql2,        &
...
!-------------------------------------------------------------------------------
   integer  im,km,id
   real  dt
   real  dzl(im,km),wwl(im,km),rql(im,km),rql2(im,km)
