...
!
#ifdef WRF
#define GWDO2D gwdo2d
#else
#define GWDO2D phys_gwd_kimarakawa
#endif
!-------------------------------------------------------------------------------
...
#if !defined(WRF) && defined(FLOW_BLOCKING)
                    omax,                                                      &
#endif
...
#else
   real                 ::  prsi(ims:ime,kms:kme+1),del(its:ite,kts:kte)
   real                 ::  oa4(ims:ime,4),ol4(ims:ime,4)
#endif
!
...
!
   ...
   real                 ::  sigma, damping
   integer              ::  kstart
   real, parameter      ::  sigma_cr = 0.005, r_day_scale = 1./(5.*86400.)
#endif
!
   logical :: iope
...
!
   ...
   real                 :: ol4p(4),olp(its:ite),od(its:ite),fr0(its:ite)
...
!
   ...
   dely   = rdely
#else
   delx     = 2.*pi*erad/ float(lonf_) / rcs
   dely     = pi*erad / float(latg_)
#endif
   dxy4(1)  = delx
   dxy4(2)  = dely
   ...
   do k = kts,kte
     do i = its,ite
       ...
       if(lat.eq.latd.and.i.eq.lond) then
         print*,'i,k',i,k,t1(i,k),q1(i,k),ro(i,k),kpbl(i)                      &
                 ,vtj(i,k),rd,fv,prsl(i,k),prslk(i,k)
       endif
#endif
     enddo
   enddo
...
#ifdef DBG
     if(lat.eq.latd.and.i.eq.lond) then
       print*,'i',i,prsi(i,1),prsi(i,kbl(i)),kbl(i),klowtop(i)
     endif
#endif
   enddo
#ifdef FLOW_BLOCKING
!
!  determine the level of maximum subgrid orographic height
!
#ifdef WRF
   komax(:) = kbl(:)
#else
...
!
   do k = kts+1,kte
     do i = its,ite
       if(kloop1(i).and.zl(i,k)-zl(i,1).ge.omax(i)) then
         komax(i) = k
         kloop1(i)  = .false.
       endif
     enddo
...
#ifdef DBG
   do i = its,ite
     print*,'check omax: omax = ',omax(i), ' 2var = ', 2.*var(i),              &
            'komax = ',komax(i), ' kbl = ',kbl(i), ' kpblmax =', kpblmax  
   enddo
#endif
#endif
#endif
...
#ifdef DBG
         if(lat.eq.latd.and.i.eq.lond) then
           print*,'i,k',i,k,del(i,k),delks(i),zl(i,k),u1(i,k),v1(i,k)
           print*,'i,k',i,k,ubar(i),vbar(i)
         endif
#endif
       endif
     enddo
...
#ifdef DBG
     if(lat.eq.latd.and.i.eq.lond) print*,'i,k',i,k,oa(i),ol(i),oc1(i),wdir,idir
#endif
#ifdef FLOW_BLOCKING 
!
...
#endif
   enddo
#if defined(FLOW_BLOCKING) && defined(DBG)
   print*, 'Flow_blocking: od, min(od) = ',minval(od),'max(od) = ',maxval(od), &
           'dxy, min(dxy) = ', minval(dxy), 'max(dxy) = ', maxval(dxy)
#endif
!                                                                       
...
#ifdef DBG
       if(lat.eq.latd.and.i.eq.lond) print*,'i,k',i,k,ti,rdz,bnv2(i,k)
#endif
     enddo                                                          
   enddo                                                             
!
...
#ifdef DBG
       if(lat.eq.latd.and.i.eq.lond) print*,'i,k step6',i,k,rulow(i),velco(i,k)
#endif
     enddo                                                          
   enddo                                                             
!                                                                       
...
!
#ifdef DBG
   if(lat.eq.latd) print*,'step 7 kpblmin kpblmax',kpblmin,kpblmax
#endif
   do k = kpblmin,kpblmax                                                
     do i = its,ite                                                    
       if (k .lt. kbl(i)) then
...
#ifdef DBG
         if(lat.eq.latd.and.i.eq.lond) print*,'i,kstep7',i,k,usqj(i,1),bnv2(i,1)
#endif
       endif
     enddo                                                          
   enddo                                                             
...
!
   do i = its,ite 
     if (.not.ldrag(i))   then   
       bnv(i) = sqrt( bnv2(i,1) )                                  
#ifdef FLOW_BLOCKING 
       fr0(i) = bnv(i)  * rulow(i) * 2. * var(i) * od(i)
       fr0(i) = min(fr0(i),frmax)
       fr(i)  = fr0(i)
#else
...
#ifdef DBG
       if(lat.eq.latd.and.i.eq.lond) then
         print*,'i,kstep7',i,k,bnv(i),fr(i),xn(i),yn(i),ldrag(i)
       endif
#endif
     endif
   enddo
...
#ifdef DBG
           if(lat.eq.latd.and.i.eq.lond) then
             print*,'step8.1',i,k,taup(i,k),hd,fro,taup(i,kp1),rim,ric
           endif
#endif
         endif
       endif
...
#ifdef DBG
            print*,'check kblk:, i=', i,'k=',k,'pe = ', pe,'ke=',ke
#endif
!
...
!
        ...
        if(kblk.ne.0) then
#ifdef DBG
          print*,'Flow_blocking: lat=',lat,'lon=',i, 'zblk=',zblk,'kblk=',kblk,&
                 'kbl=',kbl(i), 'komax=',komax(i)
#endif
!
...
!
          taufb(i,kts) = 0.5 * roll(i) * coefm(i) / dxy(i)**2 * cd * dxyp(i)   &
                         * olp(i) * zblk * ulow(i)**2
#ifdef DBG
          print*,'Flow_blocking:lat=',lat,'lon=',i,'taufb(kts) =',taufb(i,kts),&
                 'roll=',roll(i), 'coefm=',coefm(i),'dxy=',dxy(i),'cd=',cd,    &
                 'dxyp=',dxyp(i), 'olp=',olp(i),'zblk=',zblk,'ulow=',ulow(i) , &
                 'fr0=',fr0(i), 'ol=',ol(i),'od=',od(i)
#endif
          ...
          enddo
#ifdef DBG
          print*, 'Flow_blocking: lat=',lat,'lon=',i, 'taufb =', taufb(i,:)
          print*, 'Flow_blocking: lat=',lat,'lon=',i, 'before taup =', taup(i,:)
#endif
!
...
#ifdef DBG
          print*,'Flow_blocking: lat=',lat,'lon=',i, 'after taup =', taup(i,:)
        else
          print*,'Flow_blocking: no flow-blocking drag', 'lat=',lat,'lon=',i,  &
                 'zblk=',zblk, 'fr0=',fr0(i)
#endif
        endif
#ifdef DBG
     else
       print*,'Flow_blocking: no drag', 'lat=',lat,'lon=',i, 'fr0=',fr0(i)
#endif
     endif
   enddo 
#endif
...
#ifdef DBG
       if(lat.eq.latd.and.i.eq.lond) then
         print*,'step8.3',i,k,taud(i,k),taup(i,k),csg/del(i,k)
       endif
#endif
     enddo                                                             
   enddo                                                             
     ...
     do i = its,ite                                                    
       if (k .le. kbl(i)) then
         ...
         dtfac(i) = min(dtfac(i),abs(velco(i,k)                                &
                   /(deltim*rcs*taud(i,k))))
#ifdef DBG
         if(lat.eq.latd.and.i.eq.lond) print*,'step9',i,k,dtfac(i)
#endif
       endif
     enddo                                                          
   enddo                                                             
...
#ifdef DBG
       if(lat.eq.latd.and.i.eq.lond) then
         print*,' before 10',i,k,taud(i,k),dtaux,                              &
                             dtauy, dudt(i,k), dvdt(i,k), dusfc(i), dvsfc(i)
       endif
#endif
...
#ifdef DBG
       if(lat.eq.latd.and.i.eq.lond) then
         print*,' after 10',i,k,taud(i,k), dtaux,                              &
                            dtauy, dudt(i,k), dvdt(i,k), dusfc(i), dvsfc(i)
       endif
#endif
     enddo                                                          
...
#if defined(FLOW_BLOCKING) && defined(DBG)
   do i = its,ite 
     print*,'Flow_blocking:lat=',lat,'lon=',i,'ubar =',ubar(i),'dudt=',dudt(i,:)
     print*,'Flow_blocking:lat=',lat,'lon=',i,'vbar =',vbar(i),'dvdt=',dvdt(i,:)
   enddo
#endif
#ifdef DFS
#ifdef REIGH_FRICTION
!
!  apply reigh diffusion when dfs core
!
   do k = kstart,kte                                                       
     do i = its,ite
