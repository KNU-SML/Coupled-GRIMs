...
!
#ifdef NCEP2010
#define OMEGA_LAND_MASK
#undef OMEGA_DELX_FACTOR
#endif
!
#ifdef WRF
#define NCEP2010
...
!
   ...
   dtmax  = max(dt2,3600.)
!
#ifndef OMEGA_DELX_FACTOR_NEW
#ifdef NCEP2010
#ifdef OMEGA_DELX_FACTOR
   dx_factor = 25000. / delx ! assume w factor is optimized in 25 km
#else
   dx_factor = 1.
#endif
#ifdef OMEGA_LAND_MASK
   W1l = -8.E-3 * dx_factor
   W2l = -4.E-2 * dx_factor
   W3l = -5.E-3 * dx_factor
   W4l = -5.E-4 * dx_factor
...
#else /* ~WRF */
   subroutine phys_scv_hanpan(jcap,delt,del,prsl,prsi,prslk,prsik,zo,zi,       &
                 phil,ncloud,                                                  &
#ifdef DET_HYDRO
                 q2,                                                           &
#endif
...
!
   ...
   real            ::  del(ims:ime,kms:kme),                                   &
                       prsl(ims:ime,kms:kme),prslk(ims:ime,kms:kme),           &
                       prsi(ims:ime,kms:kme+1),prsik(ims:ime,kms:kme+1),       &
                       zo(ims:ime,kms:kme),phil(ims:ime,kms:kme),              &
                       zi(ims:ime,kms:kme+1)
#endif
   integer         ::  ncloud
   real            ::  slimsk(ims:ime)
#ifdef WRF
...
!
   ...
   real            ::  qlko_ktcon(ims:ime),     dellal(ims:ime,kms:kme),       &
                       dbyo(ims:ime,kms:kme),   xlamue(ims:ime,kms:kme),       &
                       ...
                       eta(ims:ime,kms:kme),    pwo(ims:ime,kms:kme)
#endif
!
#ifdef WRF
