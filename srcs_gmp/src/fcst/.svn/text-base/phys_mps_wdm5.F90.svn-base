...
!-------------------------------------------------------------------------------
   ...
   SUBROUTINE wdm52D(t, q, qci, qrs, ncr, den, p, delz                         &
                   ,delt,g, cpd, cpv, ccn0, rd, rv, t0c                        &
#else
   SUBROUTINE phys_mps_wdm5(t1, q1, q2, ncloud, n2, p, delz                    &
...
#else
         rainncv(i) = fallsum*delz(i,1)/denr*dtcld + rainncv(i)
...
#else
   SUBROUTINE wdm5init(den0,denr,dens,cl,cpv,allowed_to_read)
#endif
!-------------------------------------------------------------------------------
   IMPLICIT NONE
!-------------------------------------------------------------------------------
...
!
!#ifdef DEBUG
!        print*,' slope_wdm5 ',qr(k)*1000.,den(k),denfac(k),tk(k),tmp(k),      &
