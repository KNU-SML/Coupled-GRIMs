#include <define.h>
module rad_aerosol_data
!-------------------------------------------------------------------------------
!  ::: structure :::
!
!    [rad_aerosol_data]  This file contains 3 block data. 
!        |
!        |-- [dbaeros] *
!        |-- [dbaeros8] *
!        |-- [dbaeros99] *
...
!
!-------------------------------------------------------------------------------
   use rdparm, only   : nae,nbd
   use comswaer, only : ndm
!-------------------------------------------------------------------------------
   implicit none
!-------------------------------------------------------------------------------
!
...
!
   real                 ::  tauray(nbd)
   real                 ::  sigref(ndm,nae),haer(ndm,nae),zaer(ndm,nae)
   real                 ::  raer(nbd),oaer(ndm,nae,nbd),gaer(ndm,nae,nbd) 
! 
...
!-------------------------------------------------------------------------------
   module dbaeros8
!-------------------------------------------------------------------------------
   use rdparm8, only   : nae,nbd
   use comswaer8, only : ndm
!-------------------------------------------------------------------------------
!
!  setup common block 'swaer' for aerosols and rayleigh scattering
...
!-------------------------------------------------------------------------------
   module dbaeros99
!-------------------------------------------------------------------------------
   use rdparm99, only   : nbd
   use comswaer99, only : nae,ndm,ncm1,ncm2,ncf
!-------------------------------------------------------------------------------
!
!  setup common block 'swaer' for aerosols and rayleigh scattering
!  optical properties in eight uv+vis bands and four nir bands.
...
!
   ...
   real                 ::  sigref(ndm,nae),haer(ndm,nae),zaer(ndm,nae)
   real                 ::  raer(nbd),oaer(ndm,nae,nbd),gaer(ndm,nae,nbd)
   real                 ::  ext0(ncm1,nbd),sca0(ncm1,nbd),ssa0(ncm1,nbd)
   real                 ::  estr(nbd)
   real                 ::  aext(ncf,ncm2,nbd),bsca(ncf,ncm2,nbd),asf0(ncm1,nbd)
   real                 ::  cssa(ncf,ncm2,nbd),dasf(ncf,ncm2,nbd),abpw(ncm2,nbd)
!
   integer, private     ::  i, j, k
!
! --- tauray(nbd)
...
!
   ...
   data (((aext(i,j,k),i=1,ncf),j=1,ncm2),k=7,nbd)                             &
...
!
   data (((bsca(i,j,k),i=1,ncf),j=1,ncm2),k=1,6)                               &
    /1.431e-05,1.314e-05,4.229e-10, 2.303e-03,2.784e-03,1.272e-09,             &
     ...
     1.459e-01,1.782e-01,9.000e-08, 2.265e-04,2.462e-04,8.322e-10/
   data (((bsca(i,j,k),i=1,ncf),j=1,ncm2),k=7,nbd)                             &
...
!
   ...
   data (((cssa(i,j,k),i=1,ncf),j=1,ncm2),k=7,nbd)                             &
...
!
   ...
   data (((dasf(i,j,k),i=1,ncf),j=1,ncm2),k=7,nbd)                             &
