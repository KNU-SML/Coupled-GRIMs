...
!           |
!           |-- [post_get_rh] *
!           |-- [post_get_omega] *  <-- This routine differs with fcst's one.
!           |-- [post_get_height] *
!
!-------------------------------------------------------------------------------
...
!
!-------------------------------------------------------------------------------
   subroutine post_get_rh(im,ix,km,sl,ps,q,t,r)                                    
!-------------------------------------------------------------------------------
   use constant, only : rd_,rv_
!-------------------------------------------------------------------------------
!                                                                               
! subprogram:    post_get_rh       calculate relative humidity                        
!                                                                               
! abstract: calculates relative humidity as a function of pressure,             
!   specific humidity and moisture.  saturation specific humidity               
!   is calculated from saturation vapor pressure which is returned              
!   from a lookup table routine fpvs.                                           
!                                                                               
! program history log:                                                          
...
!                                                                               
! usage:    call post_get_rh(im,ix,km,sl,ps,q,t,r)                                    
!                                                                               
!   input argument list:                                                        
...
!                                                                               
!-------------------------------------------------------------------------------
   real             ::  sl(ix,km),ps(im),q(ix,km),t(ix,km),r(ix,km)
...
!
!-------------------------------------------------------------------------------
   subroutine post_get_omega(io,im,ix,km,si,sl,                                &
#ifdef SMP
                    w,o)                                      
#else
                    ps,psx,psy,d,u,v,o)
...
!-------------------------------------------------------------------------------
!                                                                               
! subprogram:    post_get_omega       calculate pressure vertical velocity  
!                                                                               
! abstract: calculates pressure vertical velocity post_get_omega as a function
!   of surface pressure, surface pressure gradients, and divergence             
!   and wind components on the sigma surfaces.  
!   the formula for post_get_omega is derived from the continuity equation     
...
!   92-10-31  iredell                                                           
!                                                                               
! usage:    call post_get_omega(im,ix,km,si,sl,   
!    &                 ps,psx,psy,d,u,v,o)                                      
!                                                                               
!   input argument list:                                                        
!     im       - integer number of points                                       
...
!
   ...
   real  ::  d(ix,km),u(ix,km),v(ix,km),o(ix,km)                             
   real  ::  sum(io*2)                                                      
!-------------------------------------------------------------------------------
!
   do i = 1,im                                                                 
...
!-------------------------------------------------------------------------------
!
! subprogram:    post_get_height       calculate geopotential heights
!
