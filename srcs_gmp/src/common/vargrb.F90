...
!-------------------------------------------------------------------------------
!
!  define grib surface variables and their properties.
!
!  if you are introducing new grib fields, you need to increase 
!  numgrbs and add the subsequent 9 parameters for each new field.
!
!-------------------------------------------------------------------------------
   implicit none
!-------------------------------------------------------------------------------
   integer, parameter   ::  numgrbs=59
   integer, parameter   ::  kgrbmax=100
   integer,save         ::  kgrb(numgrbs)
   integer,save         ::  kpd(25,kgrbmax,numgrbs)
   integer,save         ::  inttyp(numgrbs)
   integer,save         ::  imdata(numgrbs),jmdata(numgrbs)
   integer,save         ::  mask(numgrbs)
   logical,save         ::  limask(numgrbs)
   real,save            ::  cvalin(numgrbs)
   character(len=2),save::  condin(numgrbs)
   real,save            ::  scale(numgrbs)
   logical,save         ::  lomask(numgrbs)
   real,save            ::  cvalout(numgrbs)
   character(len=2),save::  condout(numgrbs)
   logical,save         ::  lflgrb(5,numgrbs)
   character(len=8),save::  gvar(numgrbs)
   integer              ::  igb
!
   integer,parameter    ::  igla= 1  ! glacier points
   integer,parameter    ::  imxi= 2  ! max ice extent
   integer,parameter    ::  imsk= 3  ! land sea mask
   integer,parameter    ::  iais= 4  ! sea ice mask/concentration
   integer,parameter    ::  iscv= 5  ! snow cover
   integer,parameter    ::  isno= 6  ! snow depth
   integer,parameter    ::  itsf= 7  ! sst and land skin temp
   integer,parameter    ::  iab4= 8  ! 4-type albedo
   integer,parameter    ::  iab1= 9  ! 1-type albedo
   integer,parameter    ::  ialf=10  ! albedo fraction
   integer,parameter    ::  iso2=11  ! soil moisture osu2
   integer,parameter    ::  isn1=12  ! soil moisture noa1
   integer,parameter    ::  izor=13  ! surface roughness
   integer,parameter    ::  iplr=14  ! stomato resistance
   integer,parameter    ::  itg3=15  ! deep layer soil temp
   integer,parameter    ::  ito2=16  ! soil temp osu2
   integer,parameter    ::  itn1=17  ! soil temp noa1
   integer,parameter    ::  iveg=18  ! vegetation cover
   integer,parameter    ::  ivet=19  ! vegetation type
   integer,parameter    ::  isot=20  ! soil type
   integer,parameter    ::  islo=21  ! slope type
   integer,parameter    ::  isna=22  ! snow albedo
   integer,parameter    ::  ismx=23  ! max vegetation cover
   integer,parameter    ::  ismn=24  ! min vegetation cover
!
!-vic addition
!
   integer,parameter    ::  ivgv=25  ! vic vegetation cover
   integer,parameter    ::  ivtv=26  ! vic vegetation type
   integer,parameter    ::  ivrt=27  ! root content for one vegetation type
   integer,parameter    ::  ibif=28  ! vic curve parameter (N/A)
   integer,parameter    ::  ids =29  ! Fract of dsm nonlinear baseflow begins
   integer,parameter    ::  idsm=30  ! Maximum velocity of baseflow (mm/day)
   integer,parameter    ::  iws =31  ! Fract maxi sm nonlinear baseflow occurs
   integer,parameter    ::  icef=32  ! exponent used in infiltration curve (N/A)
   integer,parameter    ::  iexp=33  ! Para the vari of Ksat with sm (N/A)
   integer,parameter    ::  ikst=34  ! Saturated hydrologic conductivity (mm/dy)
   integer,parameter    ::  idph=35  ! Soil layer thickness (m)
   integer,parameter    ::  ibub=36  ! Bubbling pressure of soil layer (cm)
   integer,parameter    ::  iqrt=37  ! Quartz content of soil layer (fraction)
   integer,parameter    ::  ibkd=38  ! Bulk density of soil layer (kg/m3)
   integer,parameter    ::  isld=39  ! Soil density of soil layer (kg/m3)
   integer,parameter    ::  iwcr=40  ! sm content at the critical point (mm)
   integer,parameter    ::  iwpw=41  ! sm content wilting point (mm)
   integer,parameter    ::  ismr=42  ! sm residual moisture (mm)
   integer,parameter    ::  islz=43  ! surface roughness of bare soil (m)
   integer,parameter    ::  isnz=44  ! surface roughness of snow pack (m)
   integer,parameter    ::  ilai=45  ! leaf area index
   integer,parameter    ::  ivg2=46  ! vic2 vegetation cover
   integer,parameter    ::  irt1=47  ! root content for layer 1
   integer,parameter    ::  irt2=48  ! root content for layer 2
   integer,parameter    ::  irt3=49  ! root content for layer 3
   integer,parameter    ::  isv1=50  ! soil moisture vic1
   ...
   integer,parameter    ::  iaer=55  ! aerosol distribution
   integer,parameter    ::  ikpr=56  ! aerosol distribution
   integer,parameter    ::  iden=57  ! aerosol distribution
   integer,parameter    ::  idxc=58  ! aerosol distribution
   integer,parameter    ::  imix=59  ! aerosol distribution
!
!  define various parameters for each field
!
!  1. number of layers (kgrb)
!
...
!
   ...
   data kgrb(iaer)/5/  ! aerosol distribution
   data kgrb(ikpr)/1/  ! aerosol distribution
   data kgrb(iden)/2/  ! aerosol distribution
   data kgrb(idxc)/5/  ! aerosol distribution
   data kgrb(imix)/5/  ! aerosol distribution
!
...
!
   ...
   data (kpd(igb,1,iaer),igb=1,25)/4*-1,191,109,    1,18*-1/ ! aerosol distribution urban
   data (kpd(igb,2,iaer),igb=1,25)/4*-1,191,109,    2,18*-1/ ! aerosol distribution continental 1
   data (kpd(igb,3,iaer),igb=1,25)/4*-1,191,109,    3,18*-1/ ! aerosol distribution marine 1
   data (kpd(igb,4,iaer),igb=1,25)/4*-1,191,109,    4,18*-1/ ! aerosol distribution continental 2
   data (kpd(igb,5,iaer),igb=1,25)/4*-1,191,109,    5,18*-1/ ! aerosol distribution marine 2
   data (kpd(igb,1,ikpr),igb=1,25)/4*-1,192,109,    1,18*-1/ ! aerosol profile type index
   data (kpd(igb,1,iden),igb=1,25)/4*-1,193,109,    1,18*-1/ ! aerosol first layer number density
   data (kpd(igb,2,iden),igb=1,25)/4*-1,193,109,    2,18*-1/ ! aerosol second layer number density
   data (kpd(igb,1,idxc),igb=1,25)/4*-1,194,109,    1,18*-1/ ! aerosol components type indices
   data (kpd(igb,2,idxc),igb=1,25)/4*-1,194,109,    2,18*-1/ ! aerosol components type indices
   data (kpd(igb,3,idxc),igb=1,25)/4*-1,194,109,    3,18*-1/ ! aerosol components type indices
   data (kpd(igb,4,idxc),igb=1,25)/4*-1,194,109,    4,18*-1/ ! aerosol components type indices
   data (kpd(igb,5,idxc),igb=1,25)/4*-1,194,109,    5,18*-1/ ! aerosol components type indices
   data (kpd(igb,1,imix),igb=1,25)/4*-1,195,109,    1,18*-1/ ! aerosol components type indices
   data (kpd(igb,2,imix),igb=1,25)/4*-1,195,109,    2,18*-1/ ! aerosol components type indices
   data (kpd(igb,3,imix),igb=1,25)/4*-1,195,109,    3,18*-1/ ! aerosol components type indices
   data (kpd(igb,4,imix),igb=1,25)/4*-1,195,109,    4,18*-1/ ! aerosol components type indices
   data (kpd(igb,5,imix),igb=1,25)/4*-1,195,109,    5,18*-1/ ! aerosol components type indices
!
...
!           =2  dominant type
!
   data inttyp(igla)/0/  ! glacier points
   data inttyp(imxi)/0/  ! max ice extent
   data inttyp(imsk)/0/  ! land sea mask (non grib on model grid)
   data inttyp(iais)/0/  ! sea ice mask/concentration
   data inttyp(iscv)/0/  ! snow cover
...
!
   ...
   data inttyp(iaer)/0/  ! aerosol distribution
   data inttyp(ikpr)/1/  ! aerosol distribution
   data inttyp(iden)/1/  ! aerosol distribution
   data inttyp(idxc)/1/  ! aerosol distribution
   data inttyp(imix)/1/  ! aerosol distribution
!
...
!   data imdata(itsf),jmdata(itsf)/ 721, 361/ ! sst and land skin temp
   data imdata(itsf),jmdata(itsf)/ 1440, 721/ ! sst and land skin temp
...
!
   ...
   data imdata(iaer),jmdata(iaer)/  360,180/ ! aerosol distribution
   data imdata(ikpr),jmdata(ikpr)/  360,180/ ! aerosol distribution
   data imdata(iden),jmdata(iden)/  360,180/ ! aerosol distribution
   data imdata(idxc),jmdata(idxc)/  360,180/ ! aerosol distribution
   data imdata(imix),jmdata(imix)/  360,180/ ! aerosol distribution
!
...
!          =2  .. use sea mask
!     if mask >0;
!       land sea mask definition of the input grib file grid (limask)
!       limask=.true.  .. use grib file field itself to define the mask
!              .false. .. use ready made 1x1 resolution land sea mask
!       if limask is .true.; 
!         additional variables, 'cvalin' and 'condin' 
!         need to be defined to obtain the land ocean mask.  cvalin is the 
!         criteria value and condin is the condition (lt,le,gt,ge,eq,ne)
!
   ...
   data limask(iais)/.true./
   data cvalin(iais),condin(iais)/0.5,'ge'/
   data mask(iscv)/0/ ! snow cover
   data limask(iscv)/.true./
   data cvalin(iscv),condin(iscv)/0.5,'ge'/
   data mask(isno)/0/ ! snow depth
   data limask(isno)/.true./
   data cvalin(isno),condin(isno)/0.5,'ge'/
   data mask(itsf)/1/ ! sst and land skin temp
   data limask(itsf)/.true./
   data cvalin(itsf),condin(itsf)/273.15,'le'/
   data mask(iab4)/0/
   data mask(iab1)/1/ ! 1-type albedo
   data limask(iab1)/.true./
   data cvalin(iab1),condin(iab1)/99.,'lt'/
   data mask(ialf)/1/ ! albedo fraction
   data limask(ialf)/.true./
   data cvalin(ialf),condin(ialf)/0.5,'ge'/
   data mask(iso2)/1/ ! soil moisture osu2
   data limask(iso2)/.false./
...
!
   ...
   data mask(iaer)/0/ ! aerosol distribution
   data mask(ikpr)/0/ ! aerosol distribution
   data mask(iden)/0/ ! aerosol distribution
   data mask(idxc)/0/ ! aerosol distribution
   data mask(imix)/0/ ! aerosol distribution
!
...
!
   ...
   data scale(iaer)/  1./ ! aerosol distribution
   data scale(ikpr)/  1./ ! aerosol distribution
   data scale(iden)/  1./ ! aerosol distribution
   data scale(idxc)/  1./ ! aerosol distribution
   data scale(imix)/  1./ ! aerosol distribution
!
!  7. flag for ouputting mask
!       lomask=.true.  .. output 1 or 0 mask
!              .false. .. regular output 
!       if lomask is .true.; 
!         additional variables, 'cvalout' and 'condout' 
!         need to be defined to obtain the output mask.  cvalout is the 
!         criteria value and condout is the condition (lt,le,gt,ge,eq,ne)
!
...
!
   ...
   data lomask(iaer)/.false./ ! aerosol distribution
   data lomask(ikpr)/.false./ ! aerosol distribution
   data lomask(iden)/.false./ ! aerosol distribution
   data lomask(idxc)/.false./ ! aerosol distribution
   data lomask(imix)/.false./ ! aerosol distribution
!
!  8.  flag for using the grib field over different land surface
!      characteristics.
!
!     lflgrb(1,:) .. open ocean 
!     lflgrb(2,:) .. land without snow
!     lflgrb(3,:) .. sea ice without snow
!     lflgrb(4,:) .. land with snow cover
!     lflgrb(5,:) .. sea ice with snow cover
!  
!   =.t. overwrites forecast sfc rec
!   =.f. keep forecast sfc rec
!     
   data (lflgrb(igb,igla),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! glacier points (nver mrgd)
   data (lflgrb(igb,imxi),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! maxice (nver mrgd)
   data (lflgrb(igb,imsk),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! land sea mask (nver mrgd)
   data (lflgrb(igb,iais),igb=1,5)/                                            &
      .true.,.false.,.true.,.false.,.true./ ! sea ice mask/conc.
   data (lflgrb(igb,iscv),igb=1,5)/                                            &
      .false.,.true.,.true.,.true.,.true./ ! snow cover
   data (lflgrb(igb,isno),igb=1,5)/                                            &
      .false.,.true.,.true.,.true.,.true./ ! snow depth
   data (lflgrb(igb,itsf),igb=1,5)/                                            &
      .true.,.false.,.false.,.false.,.false./ ! sst and skin temp
   data (lflgrb(igb,iab4),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! 4-type albedo
   data (lflgrb(igb,iab1),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! 1-type albedo
   data (lflgrb(igb,ialf),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! albedo fraction
   data (lflgrb(igb,iso2),igb=1,5)/                                            &
      .false.,.true.,.false.,.true.,.false./ ! soilm osu2
   data (lflgrb(igb,isn1),igb=1,5)/                                            &
      .false.,.true.,.false.,.true.,.false./ ! soilm noa1
   data (lflgrb(igb,izor),igb=1,5)/                                            &
      .false.,.true.,.true.,.true.,.true./ ! sfc roughness
   data (lflgrb(igb,iplr),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! stomato resistance
   data (lflgrb(igb,itg3),igb=1,5)/                                            &
      .false.,.true.,.false.,.true.,.false./ ! tg3
   data (lflgrb(igb,ito2),igb=1,5)/                                            &
      .false.,.true.,.false.,.true.,.false./ ! soil temp osu2
   data (lflgrb(igb,itn1),igb=1,5)/                                            &
      .false.,.true.,.false.,.true.,.false./ ! soil temp noa1
   data (lflgrb(igb,iveg),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! veg cover
   data (lflgrb(igb,ivet),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! veg type
   data (lflgrb(igb,isot),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! soil type
   data (lflgrb(igb,islo),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! slope type
   data (lflgrb(igb,isna),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! snow albedo
   data (lflgrb(igb,ismx),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! max veg cov
   data (lflgrb(igb,ismn),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! min veg cov
!
!-vic addition
!
   data (lflgrb(igb,ivtv),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! vic veg cover
   data (lflgrb(igb,ivgv),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! vic veg type
   data (lflgrb(igb,ivrt),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! vrt
   data (lflgrb(igb,ibif),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! binf
   data (lflgrb(igb,ids ),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! Fract of Dsm
   data (lflgrb(igb,idsm),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! Dsm
   data (lflgrb(igb,iws ),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! Ws
   data (lflgrb(igb,icef),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! c
   data (lflgrb(igb,iexp),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! expt
   data (lflgrb(igb,ikst),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! ksat
   data (lflgrb(igb,idph),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! dph
   data (lflgrb(igb,ibub),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! bubble
   data (lflgrb(igb,iqrt),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! quartz
   data (lflgrb(igb,ibkd),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! bkd
   data (lflgrb(igb,isld),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! sld
   data (lflgrb(igb,iwcr),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! wcr
   data (lflgrb(igb,iwpw),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! wpw
   data (lflgrb(igb,ismr),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! smr
   data (lflgrb(igb,islz),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! silz
   data (lflgrb(igb,isnz),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! snwz
   data (lflgrb(igb,ilai),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! lai
   data (lflgrb(igb,ivg2),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! veg cover
   data (lflgrb(igb,irt1),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! rt1
   data (lflgrb(igb,irt2),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! rt2
   data (lflgrb(igb,irt3),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! rt3
   data (lflgrb(igb,isv1),igb=1,5)/                                            &
      .false.,.true.,.false.,.true.,.false./ ! soilt vic1
   ...
   data (lflgrb(igb,iaer),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! aerosol distribution
   data (lflgrb(igb,ikpr),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! aerosol distribution
   data (lflgrb(igb,iden),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! aerosol distribution
   data (lflgrb(igb,idxc),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! aerosol distribution
   data (lflgrb(igb,imix),igb=1,5)/                                            &
      .true.,.true.,.true.,.true.,.true./ ! aerosol distribution
!
...
!
   ...
   data gvar(iaer)/'paer'/
   data gvar(ikpr)/'kprfi'/
   data gvar(iden)/'denni'/
   data gvar(idxc)/'idxci'/
   data gvar(imix)/'cmixi'/
!
